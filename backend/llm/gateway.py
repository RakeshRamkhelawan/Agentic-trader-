...
class LLMGateway:
    ...
    def __init__(
        ...
        # Performance metrics
        self._latency_stats: Dict[LLMProvider, List[float]] = {
            ...
            try:
                async with aiohttp.ClientSession() as session:
                    async with session.post(
                        ...
                        logger.info(f"✅ DeepSeek API: {'healthy' if resp.status == 200 else 'unhealthy'}")
            except Exception as e:
            ...
            try:
                async with aiohttp.ClientSession() as session:
                    async with session.get(
                        ...
                        logger.info(f"✅ OpenAI API: {'healthy' if resp.status == 200 else 'unhealthy'}")
            except Exception as e:
                logger.warning(f"⚠️ OpenAI API unavailable: {e}")
                
    async def _get_gpu_info(self) -> Optional[str]:
    ...
    def select_provider(self, requirement: LatencyRequirement, force_provider: Optional[LLMProvider] = None) -> LLMProvider:
        """
