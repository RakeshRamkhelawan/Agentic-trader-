...
class RiskGuardianAgent:
    ...
    def __init__(self, settings_service=None, memory_agent: MemoryAgent = None, message_bus=None):
        self.logger = logging.getLogger("RiskGuardian")
        self.settings_service = settings_service
        self.memory = memory_agent or MemoryAgent()
        self.message_bus = message_bus
        
    async def validate_order(self, tenant_id: str, order_payload: Dict[str, Any], user_preferences: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if profile.allowed_assets and symbol not in profile.allowed_assets:
                return {"allowed": False, "reason": f"Asset {symbol} not in whitelist", "requires_approval": True}
            ...
            if autonomy == AutonomyStatus.FULL_AUTO.value:
                return {"allowed": True, "reason": "Risk Checks Passed (Full Auto)", "requires_approval": False}
                ...
                if amount < (profile.max_order_size * 0.1):
                     return {"allowed": True, "reason": "Micro-order authorized (Semi-Auto)", "requires_approval": False}
                
                return {"allowed": False, "reason": "Semi-Auto confirmation required", "requires_approval": True}

            return {"allowed": False, "reason": "Unknown State", "requires_approval": True}

        except ValidationError as e:
            self.logger.error(f"Risk Profile Validation Error: {e}")
            return {"allowed": False, "reason": "Invalid Risk Configuration", "requires_approval": True}
        except Exception as e:
            self.logger.error(f"Guard Runtime Error: {e}")
            return {"allowed": False, "reason": f"System Error: {str(e)}", "requires_approval": True}
