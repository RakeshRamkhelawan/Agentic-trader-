...
class ExecutionGateway:
    ...
    def __init__(self, default_exchange_id: str = "kraken"):
        ...
        logger.info(f"ExecutionGateway initialized. Default: {self.default_exchange_id.upper()}. Mode: {self.trading_mode.upper()}")
    ...
    async def execute_order(self, symbol: str, side: str, amount: float, order_type: str = "market", price: Optional[float] = None, target_exchange: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        logger.info(f"Execution Request: {side.upper()} {amount} {symbol} @ {order_type} on {target.upper()} ({self.trading_mode.upper()})")
