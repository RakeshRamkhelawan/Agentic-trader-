...
from backend.models.user_settings import (
    ...
    UserSecurity as DBUserSecurity,
...
class UserSettingsService:
    ...
    async def get_appearance(
        ...
        return AppearanceSettings(theme=db_prefs.theme or "system")

    async def update_appearance(
        self, db: AsyncSession, tenant_id: str, appearance: AppearanceSettings
    ) -> AppearanceSettings:
        user = await self._get_or_create_user(db, tenant_id)
        result = await db.execute(
            select(DBUserPreferences).where(DBUserPreferences.user_id == user.id)
    ...
    async def update_preferences(
        self, db: AsyncSession, tenant_id: str, prefs: UserPreferences
    ) -> UserPreferences:
        user = await self._get_or_create_user(db, tenant_id)
        result = await db.execute(
            select(DBUserPreferences).where(DBUserPreferences.user_id == user.id)
    ...
    async def add_api_key(
        self, db: AsyncSession, tenant_id: str, request: BrokerAPIKeyCreate
    ) -> BrokerAPIKey:
        user = await self._get_or_create_user(db, tenant_id)
        ...
        pass_enc = (
            cipher_suite.encrypt(request.passphrase.encode()).decode()
    ...
    async def delete_api_key(self, db: AsyncSession, tenant_id: str, key_id: str):
        ...
        user = await self._get_or_create_user(db, tenant_id)
        result = await db.execute(
            select(DBAPIKey).where(DBAPIKey.id == key_id, DBAPIKey.user_id == user.id)
        )
