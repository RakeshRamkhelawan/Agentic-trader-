...
class OrderExecutor:
    ...
    def __init__(
        ...
        if adapter_type == "ExchangeAdapter":
            self.logger.warning(
                "[WARNING] Using MOCK ExchangeAdapter - orders will NOT be placed on real exchange!"
            )
        else:
            self.logger.info(f"[SUCCESS] Using {adapter_type} for order execution")

    async def execute_trade(self, execution_plan: ExecutionPlan) -> ExecutionOutcome:
        """
    ...
    async def _wait_for_fill(self, order: Order, timeout: int) -> Order:
        """
        ...
        while (datetime.now(UTC) - start_time).total_seconds() < timeout:
            ...
            if updated_order.status == "filled":
                ...
                raise ExecutionError(f"Order {order.order_id} {updated_order.status}")
        ...
        raise ExecutionError(f"Order {order.order_id} timed out after {timeout}s")
