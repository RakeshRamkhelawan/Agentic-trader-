...
class OrderResult:
    ...
    def __init__(self, order_id: str, status: OrderStatus, filled_qty: float = 0.0, avg_price: float = 0.0, error_message: str = ""):
...
class ShadowPortfolioManager:
    ...
    async def submit_order(self, order: OrderRequest) -> OrderResult:
        ...
        if side == OrderSide.BUY:
            ...
            if cash < cost:
                return OrderResult(str(uuid.uuid4()), OrderStatus.REJECTED, error_message="Insufficient EUR")
            ...
            if pos < qty:
                return OrderResult(str(uuid.uuid4()), OrderStatus.REJECTED, error_message=f"Insufficient {symbol}")
