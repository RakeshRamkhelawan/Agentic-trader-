...
class OrderRiskValidator:
    ...
    def _validate_balance(
        ...
        if hasattr(balance, 'free'):
            available = balance.free if isinstance(balance.free, Decimal) else Decimal(str(balance.free))
        else:
            available = Decimal(str(balance)) if balance else Decimal("0")

        if request.side.value == "buy":
            required = request.quantity * (request.price or request.expected_price) * Decimal("1.01")  # 1% buffer
            ...
            if request.price:
                ...
                if deviation > self.limits.price_deviation_pct:
                    checks.append(ValidationCheck(
                        ...
                        details={"order_price": float(request.price), "market_price": float(last)}
                    ))
                else:
                    checks.append(ValidationCheck(
                        name="price_sanity",
