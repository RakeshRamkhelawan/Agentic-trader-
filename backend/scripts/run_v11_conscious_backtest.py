...
def run_v11_conscious_backtest() -> Dict[str, Any]:
    ...
    print("  [AHAMKARA] Self-awareness active")
    ...
    for day_idx, date in enumerate(sorted_dates):
            ...
            if pos.position != 0 and sym in current_prices:
                ...
                if exit_reason:
                    ...
                    trade_exp = TradeExperience(
                        ...
                        net_pnl=net_pnl,
                        return_pct=(
                            net_pnl / (abs(pos.position) * pos.entry_price)
                            if pos.position != 0
                            else 0
                        ),
        ...
        for sym, pos in positions.items():
            if (
                ...
                conscious_decision = ahamkara.decide_action(market_state, decision, memory_insights)
                ...
                if final_size >= 200:
                    ...
                    conscious_context = {
                        ...
                        "guna_dominant": (
                            getattr(decision, "guna_state", {}).dominant()
                            if hasattr(getattr(decision, "guna_state", {}), "dominant")
                            else "unknown"
                        ),
