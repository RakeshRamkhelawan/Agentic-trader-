...
class GunaVector:
    ...
    def to_dict(self) -> Dict[str, float]:
        return {"sattva": self.sattva, "rajas": self.rajas, "tamas": self.tamas, "dominant": self.dominant()}
...
def download_data(platform_symbol: str, yf_ticker: str, start: str = START_DATE, end: str = END_DATE) -> List[Dict]:
...
class TechnicalAnalyzer:
    ...
    def analyze(self, symbol: str, prices: List[float], volumes: List[float], 
                highs: List[float], lows: List[float]) -> MarketState:
        ...
        momentum_1w = (prices[-1] - prices[-min(7, len(prices))]) / prices[-min(7, len(prices))] if len(prices) >= 2 else 0
        recent = prices[-20:]
        ...
        return MarketState(
            symbol=symbol, price=current_price, close=current_price, volume=volumes[-1],
            rsi=rsi, rsi_4h=rsi_4h, rsi_1d=rsi_1d, adx=adx, atr=atr, volatility=volatility,
            trend_1h=trend_1h, trend_4h=trend_4h, trend_1d=trend_1d,
            ema_8=ema_8, ema_21=ema_21, ema_55=ema_55, sma_50=sma_50, sma_200=sma_200,
            momentum_1d=momentum_1d, momentum_1w=momentum_1w,
...
class MayaDetector:
    ...
    def detect(self, signals: List[AgentSignal], market: MarketState, 
               coherence: float, guna: GunaVector) -> Tuple[bool, str]:
        """
        Returns: (is_maya, reason)
        """
        # Criterion 1: Low coherence in volatile market
        if coherence < self.params.maya_coherence_threshold and market.volatility > self.params.maya_volatility_threshold:
            return True, f"Low coherence ({coherence:.2f}) in volatile market ({market.volatility:.3f})"
        
        # Criterion 2: Chaotic guna state (high Rajas, low Sattva)
        if guna.rajas > self.params.maya_rajas_threshold and guna.sattva < self.params.maya_sattva_threshold:
            return True, f"Chaotic state: High Rajas ({guna.rajas:.2f}), Low Sattva ({guna.sattva:.2f})"
        ...
        trend_aligned = (market.rsi > 50 and market.trend_1d > 0) or (market.rsi < 50 and market.trend_1d < 0)
...
class AirAgent(ElementalAgent):
    ...
    def __init__(self, params: HyperParameters):
        super().__init__("Air_Regime", ElementType.AIR, {"sattva": 0.4, "rajas": 0.4, "tamas": 0.2}, params)
    
    def analyze(self, market: MarketState) -> AgentSignal:
        if not self.consume_prana():
            return AgentSignal(self.name, self.element, ActionType.HOLD, 0.3, 0.0, "Low prana", {"depleted": True})
        ...
        return AgentSignal(self.name, self.element, action, round(min(0.95, confidence), 3),
                          round(final_strength, 3), f"Air: {regime}, {sentiment}",
...
class FireAgent(ElementalAgent):
    ...
    def __init__(self, params: HyperParameters):
        super().__init__("Fire_Momentum", ElementType.FIRE, {"sattva": 0.25, "rajas": 0.6, "tamas": 0.15}, params)
    ...
    def analyze(self, market: MarketState) -> AgentSignal:
        ...
        return AgentSignal(self.name, self.element, action, round(min(0.95, confidence), 3),
                          round(final_strength, 3), f"Fire: mom={momentum_score:.2f}, OBV={obv_signal}",
...
class WaterAgent(ElementalAgent):
    ...
    def __init__(self, params: HyperParameters):
        super().__init__("Water_Trend", ElementType.WATER, {"sattva": 0.5, "rajas": 0.35, "tamas": 0.15}, params)
    ...
    def analyze(self, market: MarketState) -> AgentSignal:
        ...
        return AgentSignal(self.name, self.element, action, round(min(0.95, confidence), 3),
                          round(final_strength, 3), f"Water: {structure_type}",
                          {"structure": structure_type, "trend_alignment": trend_alignment, "ema_aligned": ema_bullish or ema_bearish})
...
class EarthAgent(ElementalAgent):
    ...
    def __init__(self, params: HyperParameters):
        super().__init__("Earth_Execution", ElementType.EARTH, {"sattva": 0.35, "rajas": 0.15, "tamas": 0.5}, params)
    ...
    def analyze(self, market: MarketState) -> AgentSignal:
        ...
        confidence = 0.4 + abs(final_strength) * 0.4 + (0.15 if near_support or near_resistance else 0)
        ...
        return AgentSignal(self.name, self.element, action, round(min(0.95, confidence), 3),
                          round(final_strength, 3), f"Earth: {zone}",
...
class EtherOrchestrator:
    ...
    def harmonize(self, signals: List[AgentSignal], market: MarketState) -> CollectiveDecision:
        ...
        if not signals:
            return CollectiveDecision(ActionType.HOLD, 0.0, 0.0, 0.0, 0.0, [], 
                                     ElementType.ETHER, None, GunaVector(), "No signals", True)
        
        # Calculate weighted strength
        total_conf = sum(s.confidence for s in signals)
        weighted_strength = sum(s.strength * s.confidence for s in signals) / total_conf if total_conf > 0 else 0
        ...
        dominant = max(element_scores, key=element_scores.get) if element_scores else ElementType.ETHER
        ...
        is_maya, maya_reason = self.maya_detector.detect(signals, market, coherence, collective_guna)
        ...
        decision = CollectiveDecision(action, confidence, coherence, harmony, weighted_strength,
                                     [s.agent_name for s in signals], dominant, suppressed,
                                     collective_guna, rationale, is_maya)
    ...
    def _decide(self, harmony: float, is_maya: bool, maya_reason: str, 
                strength: float, dominant: ElementType) -> Tuple[ActionType, str]:
        ...
        if strength > self.params.consensus_threshold:
            return ActionType.BUY, f"BUY: strength={strength:.2f}, harmony={harmony:.2f}, {dominant.value}"
        elif strength < -self.params.consensus_threshold:
            return ActionType.SELL, f"SELL: strength={strength:.2f}, harmony={harmony:.2f}, {dominant.value}"
...
class RiskManager:
    ...
    def __init__(self, max_positions: int = 5, max_per_sector: int = 2, params: HyperParameters = DEFAULT_PARAMS):
    ...
    def calculate_size(self, capital: float, decision: CollectiveDecision, atr: float, price: float) -> float:
...
class BacktestEngine:
    ...
    def run(self, data: Dict[str, List[Dict]], dates: List[str]) -> Dict[str, Any]:
        ...
        price_history = {sym: {"prices": [], "volumes": [], "highs": [], "lows": []} for sym in data.keys()}
        ...
        for date in dates:
                ...
                if pos.position != 0 and sym in current_prices:
                    ...
                    if reason:
                        ...
                        if pnl > 0:
                            metrics[sym]["wins"] += 1
                        metrics[sym]["pnl"] += pnl
                        trades.append(TradeRecord(date, sym, "close", pos.side or "unknown", pnl, 
                                                 pos.entry_price, bar["close"], reason))
            ...
            for sym, pos in positions.items():
                if pos.position == 0 and sym in current_prices and len(price_history[sym]["prices"]) >= 60:
                    ...
                    ph = price_history[sym]
                    market = self.analyzer.analyze(sym, ph["prices"], ph["volumes"], ph["highs"], ph["lows"])
                    ...
                    if self.risk.can_open(sym, sector, decision) and decision.confidence >= self.params.min_confidence:
                        if decision.action in [ActionType.BUY, ActionType.SELL]:
                            pos_size = self.risk.calculate_size(capital, decision, market.atr, market.price)
                            if pos_size >= 200:
                                ...
                                trades.append(TradeRecord(date, sym, "open", side, 0.0, market.price, market.price, 
    ...
    def _calculate_metrics(self, equity: List[float], trades: List[TradeRecord], 
                          symbol_metrics: Dict) -> Dict[str, Any]:
...
def grid_search_optimization(data: Dict[str, List[Dict]], dates: List[str], 
                             param_grid: Dict[str, List]) -> Tuple[HyperParameters, Dict]:
    """
    ...
    for combo in itertools.product(*values):
        ...
        if sharpe > best_sharpe and metrics["total_trades"] > 50:
            ...
            print(f"  New best: Sharpe={sharpe:.2f}, Return={metrics['total_return_pct']:.1f}%, Trades={metrics['total_trades']}")
...
def run_v9_backtest(optimize: bool = False):
    ...
    for sector, syms in UNIVERSE_GROUPS.items():
        s_trades = sum(metrics['symbol_metrics'].get(s, {}).get("trades", 0) for s in syms if s in metrics['symbol_metrics'])
        s_wins = sum(metrics['symbol_metrics'].get(s, {}).get("wins", 0) for s in syms if s in metrics['symbol_metrics'])
        s_pnl = sum(metrics['symbol_metrics'].get(s, {}).get("pnl", 0) for s in syms if s in metrics['symbol_metrics'])
    ...
    report = {
        ...
        "sector_performance": {
            sector: {
                "trades": sum(metrics['symbol_metrics'].get(s, {}).get("trades", 0) for s in syms if s in metrics['symbol_metrics']),
                "pnl": sum(metrics['symbol_metrics'].get(s, {}).get("pnl", 0) for s in syms if s in metrics['symbol_metrics'])
            }
            for sector, syms in UNIVERSE_GROUPS.items()
        }
    }
