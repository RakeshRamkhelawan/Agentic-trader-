...
async def run_v6_backtest():
    ...
    for date in sorted_dates:
        ...
        for sym, tracker in trackers.items():
            if (
                ...
                if risk_manager.can_open(sym, sector):
                    ...
                    # Entry Filters
                    if market_data["regime"] == "SIDEWAYS" or market_data["adx"] < 20:
                        continue
                    ...
                    if (
                        market_data["trend"] > 0
                        and market_data["rsi"] < 65
                        and market_data["ema_bullish"]
                    ):
                        action = "bullish"
                    elif (
                        market_data["trend"] < 0
                        and market_data["rsi"] > 35
                        and market_data["ema_bearish"]
                    ):
                        action = "bearish"
        ...
        for tracker in trackers.values():
            if tracker.position != 0:
                ...
                last_p = price_history[next(s for s, t in trackers.items() if t == tracker)][
                    "prices"
                ][-1]
    ...
    print("\n" + "=" * 80)
    print("  v6 RESULTS (Portfolio Mode)")
    print(f"  Total PnL: ${total_pnl:,.2f} | Return: {final_return:.2f}%")
    print(
        f"  Total Trades: {total_trades} | Avg PnL/Trade: ${total_pnl/total_trades if total_trades else 0:,.2f}"
    )
    print("=" * 80)
