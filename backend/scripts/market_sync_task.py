...
async def sync_kraken(cache):
    ...
    try:
        ...
        symbols = ["BTC/EUR", "ETH/EUR", "SOL/EUR", "ADA/EUR", "DOT/EUR", "XRP/EUR", "LINK/EUR", "DOGE/EUR"]
...
async def sync_revolut(cache):
    ...
    try:
        ...
        if isinstance(instruments, dict):
            ...
            items = [(inst if isinstance(inst, str) else inst.get('symbol'), inst) for inst in instruments]
        else:
        ...
        if markets_data:
            ...
            logger.info(f"Fetching tickers for {len(raw_symbols)} Revolut symbols (excluded {len(blacklist)} toxic)...")
            ...
            logger.info(f"Synced {len(markets_data)} symbols from Revolut ({matched_count} updated live)")
        else:
