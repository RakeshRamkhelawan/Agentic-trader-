...
class MockVedAstroOrchestrator:
    ...
    def generate_daily_plan(
        self, date_str: str, symbols: List[str], market_data: Dict[str, Any]
    ) -> DailyTradingPlan:
        """
        ...
        if is_favorable and muhurtha_rating >= 7:
            ...
            trading_windows = [
                ...
                TimeWindow(
                    ...
                    TradingWindow.GOOD,
                    0.9,
                    "Afternoon Ruling period",
                    "Exit/Trim",
                ),
                TimeWindow(
                    time_class(15, 0),
                    time_class(17, 0),
                    TradingWindow.MODERATE,
                    0.7,
...
def run_v10_vedastro_backtest():
        ...
        if date != current_date:
            ...
            for sym in all_data.keys():
                ...
                if day_bar and len(price_history[sym]["prices"]) >= 50:
                    ...
                    ms = tech_analyzer.analyze_market_state(
            ...
            # Generate daily plan
            if market_snapshot:
                daily_plan = vedastro.generate_daily_plan(
                    date, list(all_data.keys()), market_snapshot
                )
                top_assets = [a.symbol for a in daily_plan.top_assets]
                print(
                    f"    {date}: Top assets: {top_assets}, Rating: {daily_plan.muhurtha_rating:.1f}/10"
                )
        ...
        for sym, pos in positions.items():
            if (
                ...
                if risk_manager.can_open(sym, sector):
                    ph = price_history[sym]
                    market_state = tech_analyzer.analyze(
                        sym, ph["prices"], ph["volumes"], ph["highs"], ph["lows"]
                    )
    ...
    print(
        f"    Filter Rate:      {vedastro_blocked_trades / (vedastro_allowed_trades + vedastro_blocked_trades) * 100:.1f}%"
    )
    ...
    report = {
        ...
        "vedastro_stats": {
            ...
            "filter_rate_pct": vedastro_blocked_trades
            / (vedastro_allowed_trades + vedastro_blocked_trades)
            * 100,
        },
