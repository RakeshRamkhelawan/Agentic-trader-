...
from backend.scripts.run_v8_symbiotic_backtest import (
    run_v8_symbiotic_backtest,  # Baseline
    TechnicalAnalyzer,
...
class StrategicPositionSizer:
    ...
    def calculate_size(
        self, capital: float, decision: dict, atr: float, price: float, 
        size_mult: float = 1.0
    ) -> float:
...
def run_v9_integrated_backtest(
    ...
    if use_strategic:
        ...
        mcts_planner = StrategicMCTSPlanner(
    ...
    if use_strategic:
        ...
        for plan_date in plan_dates[:10]:  # First 10 for demo
            ...
            if snapshot:
                ...
                plan = mcts_planner.plan(
                    portfolio, snapshot, list(snapshot.keys())
                )
               ...
               len(price_history[sym]["prices"]) >= 60:
                ...
                if use_strategic:
                    ...
                    pos_size = strategic_sizer.calculate_size(
                        capital, {"action": decision.action}, market_state.atr, 
                        market_state.price, size_mult
                    )
                else:
                    # v8 sizing
