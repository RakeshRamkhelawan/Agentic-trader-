...
from backend.scripts.run_v8_symbiotic_backtest import (
    ...
    ElementalAgent, EtherAgent, AirAgent, FireAgent, WaterAgent, EarthAgent
)
            ...
            if pos.position == 0 and sym in current_prices and \
               len(price_history[sym]["prices"]) >= 60:
                ...
                audit_logger.log_risk_check(
                    ...
                    passed=can_open,
                    rejection_reason="" if can_open else f"Harmony: {decision.harmony_score:.2f}, Maya: {decision.is_maya}"
                )
