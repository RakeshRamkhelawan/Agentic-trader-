...
class ElementalBase(BaseAgent, ABC):
    ...
    def _register_with_identity(self):
        ...
        try:
            ...
            logger.info(f"Registered {self.agent_name} with SystemIdentity layer {self.tattva_layer}")
    ...
    async def regenerate_prana(self, rest_period_seconds: int) -> float:
        ...
        if rest_period_seconds > 3600:
             ...
             recovery_amount = (rest_period_seconds / 3600.0) * 10.0 # Slower recovery for short naps
        ...
        logger.info(f"{self.agent_name} regenerated {recovery_amount:.1f} prana. Level: {self.prana:.1f}")
        return self.prana
