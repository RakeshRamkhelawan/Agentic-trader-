...
class EarthAgentV6:
    ...
    def valuate(self, symbol: str, current_price: float, 
                prices: List[float], market_regime: str) -> EarthValuation:
        ...
        if len(prices) < 30:
            return EarthValuation(
                ...
                verdict="FAIR", confidence=self.calculate_confidence(symbol, {}),
                methodology="Onvoldoende data", earth_dharma="Prithvi wacht"
            )
...
class FireAgentV6:
    ...
    def assess(self, symbol: str, proposed_action: str, proposed_qty: float,
               price: float, portfolio_value: float, navagraha: NavagrahaState,
               harmony_score: float, prana_level: float, market_regime: str,
        ...
        if dynamic_confidence < 0.30:
            return FireDecision(
                decision="BLOCK", confidence=dynamic_confidence, max_allowed_qty=None,
                risk_score=0.8, blocking_reasons=[f"{symbol} loss streak: {self.loss_streaks[symbol]}"],
                var_estimate_pct=0, fire_dharma=f"Agni: {symbol} te riskant door verliesreeks",
                prana_consumed=PRANA_COSTS["risk"]
            )
        ...
        return FireDecision(
            decision=decision, confidence=dynamic_confidence,
...
class WaterAgentV6:
    ...
    def calculate_confidence(self, symbol: str, macro_signal: Dict, 
    ...
    def analyze_regime(self, symbol: str, market_data: Dict[str, Any],
                       navagraha: NavagrahaState, prices: List[float]) -> WaterRegime:
        ...
        if len(prices) < 20:
            return WaterRegime(
                ...
                confidence=0.50, water_dharma="Water is stil"
            )
        ...
        # Build macro signal
        macro_signal = {
            "risk_on_score": 0.6 if regime == "expansion" else 0.4 if regime == "contraction" else 0.5,
...
class AirAgentV6:
    ...
    def generate_signals(self, symbol: str, current_price: float,
        ...
        if len(prices) < 20:
            return AirSignal(
                symbol=symbol, action="HOLD", confidence=0.50,
        ...
        indicators = {
            "rsi": round(rsi, 2), "ema20": round(ema20, 2),
            "ema50": round(ema50, 2), "atr_pct": round(atr_pct, 2),
...
class EtherOrchestratorV6:
    ...
    def synthesize(self, fire: FireDecision, water: WaterRegime,
                   air: AirSignal, earth: EarthValuation,
                   navagraha: NavagrahaState) -> Tuple[float, bool]:
...
class ElementalAgentManagerV6:
    ...
    def process_trading_cycle(self, symbol: str, current_price: float,
        ...
        water = self.water_agent.analyze_regime(
            symbol, {}, navagraha, prices
        )
        
        air = self.air_agent.generate_signals(
            symbol, current_price, navagraha, prices
        )
        
        earth = self.earth_agent.valuate(
            symbol, current_price, prices, water.regime
        )
        ...
        fire = self.fire_agent.assess(
            symbol=symbol, proposed_action=air.action, proposed_qty=0.01,
            price=current_price, portfolio_value=portfolio_value, 
            navagraha=navagraha, harmony_score=base_harmony,
            prana_level=prana_level, market_regime=water.regime,
            volatility_24h=volatility
        )
        
        # Ether synthesizes
        harmony, consensus = self.ether_orchestrator.synthesize(
            fire, water, air, earth, navagraha
        )
        ...
        if not should_trade:
            return EtherSynthesis(
                ...
                execution_urgency="none", consensus_achieved=consensus,
                blocking_agent="ether" if harmony < HARMONY_THRESHOLDS.get(navagraha.dominant_planet, 0.50) else None,
        ...
        return EtherSynthesis(
            final_decision="EXECUTE", harmony_score=harmony,
            approved_symbol=symbol,
            approved_action=air.action,
            approved_qty=approved_qty, approved_price=current_price,
