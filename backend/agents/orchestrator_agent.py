...
from backend.core.schemas.ooda_types import Observation, Orientation, TradeProposal, RiskAssessment, ResearchHypothesis
...
class OrchestratorAgent(BaseAgent):
    ...
    async def harmonize(
        ...
        # 1. Risk Check
        all_approved = all(r.decision.value == "approve" for r in risk_assessments) if risk_assessments else True
        ...
        if orientation.regime == "trending_up" and bear_hypothesis and bear_hypothesis.confidence > 0.8:
        ...
        if orientation.regime == "trending_down" and bull_hypothesis and bull_hypothesis.confidence > 0.8:
            self.harmony_score -= 0.1
