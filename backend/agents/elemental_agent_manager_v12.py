"""
Elemental Agent Manager V12 - HEDGE PAIRS + BOND INVERSE + SENTIMENT

V12 Key Changes:
- Earth: should_enter() - block entry na 4 consecutive losses
- Water: Bond inverse logica (TLT, IEF, AGG, BND, TBF, GOVT)
- Water: Hedge pairs (SH/SPY, PSQ/QQQ, RWM/IWM, TBF/TLT)
- Air: LLM sentiment integratie (optioneel)
- 54-asset universe (50 + 4 inverse ETFs)

Baseline: V10 (geen crypto decay van v11)
"""
...
logger = logging.getLogger("ElementalAgentsV12")
...
class EarthAgentV12:
    ...
    def should_enter(self, symbol: str) -> bool:
        """
        V12: Earth blokkeert entry als recente performance structureel slecht is.
        Wordt aangeroepen VOOR BUY-beslissing in engine entry-loop.
        Geen hardcoded symbolen — puur geheugen-gebaseerd.
        """
        recent = list(self.symbol_memory.get(symbol, []))
        if len(recent) >= 4:
            recent_wr = sum(1 for t in recent[-4:] if t["win"]) / 4
            if recent_wr == 0.0:
                return False  # 0% WR laatste 4 → Earth vertrouwt niet
        return True
...
class AirAgentV12:
    ...
    def inject_sentiment(
        self, symbol: str, score: float, date: datetime, source: str = "llm"
    ):
        """
    ...
    def calculate_confidence(
        self, symbol: str, current_date: Optional[datetime] = None
    ) -> float:
        """
...
class ElementalAgentManagerV12:
