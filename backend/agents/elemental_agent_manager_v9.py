...
PLANET_THRESHOLDS = {
    "SUN":     0.58, "MOON":    0.56, "MARS":    0.63,
    "MERCURY": 0.57, "JUPITER": 0.60, "VENUS":   0.57,
    "SATURN":  0.53, "RAHU":    0.65, "KETU":    0.61,
}
...
class ElementalAgentManagerV9:
    ...
    def evaluate_open_position(self, symbol: str, current_price: float,
                               macro_signal: MacroSignal) -> Tuple[bool, str]:
        """
        ...
        if symbol in self.earth_agent.entry_confidences:
            ...
            if current_conf < entry_conf * 0.75:
                logger.debug(f"{symbol}: Earth would EXIT (entry_conf={entry_conf:.3f}, current={current_conf:.3f})")
