...
class ElementalAgentManager:
    ...
    def water_agent_analyze(self,
        ...
        if regime == "contraction":
            avoid = [s for s in prices.keys() if self._get_asset_info(s) and 
                    self._get_asset_info(s).asset_class.value == "crypto"]
        ...
        narratives = {
            ...
            "contraction": f"Contractie fase - defensieve positie aangeraden",
            "recovery": f"Herstel bezig - selectieve kansen in {dominant} assets",
            "neutral": f"Neutrale markt - wachten op duidelijke richting"
        }
    ...
    def air_agent_generate_signals(self,
                                  symbol: str,
                                  current_price: float,
    ...
    def ether_agent_synthesize(self,
                              fire: FireDecision,
                              water: WaterRegime,
                              air: AirSignal,
        ...
        harmony_score = sum(
            confidences[agent] * guna_weights[agent] 
        ...
        if fire.decision == "BLOCK":
            return EtherSynthesis(
                ...
                blocking_agent="fire",
                cosmic_narrative=f"Agni heeft gesproken - veiligheid boven winst",
