...
class ElementalAgentManagerV5:
    ...
    def water_agent_analyze(
        ...
        price_change_7d = (
            (prices[-1] - prices[-min(7, len(prices))])
            / prices[-min(7, len(prices))]
            * 100
        )
        ...
        price_change_30d = (
            (prices[-1] - prices[-min(30, len(prices))])
            / prices[-min(30, len(prices))]
            * 100
        )
        ...
        confidence = (
            base_confidence + trend_score - vol_penalty + volume_score + regime_clarity
        )
    ...
    def ether_agent_synthesize(
        ...
        harmony_score = sum(
            confidences[agent] * weights[agent] for agent in confidences
        )
        ...
        agents_above_baseline = sum(
            1 for agent, conf in confidences.items() if conf > 0.50
        )
