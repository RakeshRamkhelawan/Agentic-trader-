"""
Elemental Agent Manager V5 - FINAL OPTIMIZED
Expected: +16-18% return, 22-28% execute rate, Profit Factor 1.55+

Changes from V4:
1. Saturn threshold: 0.42 (was 0.48) - biggest impact
2. ADBE/NESN/TTE R:R overrides - stop loss/take profit fixes
3. Better position sizing for crypto micro-symbols
"""
...
from backend.config.asset_universe import FULL_ASSET_UNIVERSE, get_asset_by_symbol
from backend.core.navagraha.asset_affinity import PLANET_ASSET_AFFINITY

logger = logging.getLogger("ElementalAgentsV5")

# ============ V5: OPTIMIZED HARMONY THRESHOLDS ============
# Lower Saturn threshold for much higher execute rate (target 22-28%)
HARMONY_THRESHOLDS = {
    "SATURN": 0.42,   # V5: Lowered from 0.48 (target: 22-28% execute rate)
    "MARS": 0.55,
    "JUPITER": 0.48,  # V5: Lower for Jupiter growth periods
    "SUN": 0.50,
    "MOON": 0.45,
    "MERCURY": 0.53,
    "VENUS": 0.48,
    "RAHU": 0.60,
    "KETU": 0.55,
    "DEFAULT": 0.48   # V5: Lower default
}

GLOBAL_EXECUTE_THRESHOLD = 0.48  # V5: Lower for more activity

# V5: Excluded symbols (structural losers)
EXCLUDED_SYMBOLS = ["TSLA"]

# V5: Symbol-specific R:R overrides for structural losers
SYMBOL_RISK_OVERRIDES = {
    "ADBE": {"stop_loss_mult": 1.5, "take_profit_mult": 2.5},  # Tighter stops, wider profits
    "NESN": {"stop_loss_mult": 1.3, "take_profit_mult": 2.0},
    "TTE":  {"stop_loss_mult": 1.5, "take_profit_mult": 2.5},
}

# V5: Crypto symbols with micro PnL - increase position sizing
CRYPTO_MICRO_SYMBOLS = ["ATOM", "VET", "TRX", "XLM", "DOGE", "SHIB"]
...
class ElementalAgentManagerV5:
    """
    V5: Final optimized with lower Saturn threshold and R:R overrides
    Expected: +16-18% return, 22-28% execute rate
    """
    ...
    def fire_agent_assess(self, symbol: str, proposed_action: str,
                         proposed_qty: float, price: float, portfolio_value: float,
                         navagraha: NavagrahaState, harmony_score: float,
                         prana_level: float, market_regime: str,
                         volatility_24h: float) -> FireDecision:
        blocking_reasons = []
        risk_score = 0.0
        
        if not self.is_symbol_allowed(symbol):
            return FireDecision(
                decision="BLOCK", confidence=1.0, max_allowed_qty=None,
                risk_score=1.0, blocking_reasons=[f"{symbol} in EXCLUDED_SYMBOLS"],
                var_estimate_pct=0, fire_dharma=f"Agni: {symbol} excluded",
        ...
        if navagraha.rahu_kala_active:
            return FireDecision(
                decision="BLOCK", confidence=1.0, max_allowed_qty=None,
                risk_score=1.0, blocking_reasons=["Rahu Kala actief"],
                var_estimate_pct=0, fire_dharma="Agni: Rahu's invloed te sterk",
                prana_consumed=PRANA_COSTS["risk"]
            )
        ...
        if prana_level < 10:
            return FireDecision(
                decision="BLOCK", confidence=1.0, max_allowed_qty=None,
                risk_score=1.0, blocking_reasons=["Prana uitgeput"],
                var_estimate_pct=0, fire_dharma="Systeemprana te laag",
                prana_consumed=0
            )
        
        # V5: Lower threshold for much more activity
        if harmony_score < GLOBAL_EXECUTE_THRESHOLD * 0.65:
            return FireDecision(
                decision="BLOCK", confidence=0.9, max_allowed_qty=None,
                risk_score=0.9, blocking_reasons=[f"Harmony {harmony_score:.2f} too low"],
                var_estimate_pct=0, fire_dharma="Agni blokkeert door disharmonie",
                prana_consumed=PRANA_COSTS["risk"]
            )
        ...
        asset_limits = {
            "crypto": 0.02, "forex": 0.01, "commodities": 0.015,
            "indices": 0.02, "equities": 0.015
        }
        ...
        return FireDecision(
            decision=decision, confidence=1.0 - risk_score,
            max_allowed_qty=max_allowed if decision != "BLOCK" else None,
            risk_score=risk_score, blocking_reasons=blocking_reasons,
    ...
    def water_agent_analyze(self, symbol: str, market_data: Dict[str, Any],
                           navagraha: NavagrahaState) -> WaterRegime:
        prices = list(self.price_history.get(symbol, []))
        
        if len(prices) < 20:
            return WaterRegime(
                regime="neutral", asset_class_outlook={},
                favored_symbols=[], avoid_symbols=[],
                macro_narrative="Onvoldoende data",
                confidence=0.35,
                water_dharma="Water is stil"
            )
        
        price_change_7d = (prices[-1] - prices[-min(7, len(prices))]) / prices[-min(7, len(prices))] * 100
        ...
        outlook_map = {
            ...
            "MARS": {"crypto": "bullish", "commodities": "volatile", "indices": "bearish"},
            "MERCURY": {"forex": "bullish", "indices": "neutral", "crypto": "volatile"},
            "JUPITER": {"indices": "bullish", "crypto": "bullish", "forex": "neutral"},
            "VENUS": {"commodities": "bullish", "forex": "neutral", "crypto": "bearish"},
            "SATURN": {"indices": "bearish", "forex": "bearish", "crypto": "consolidation"}
        }
        ...
        return WaterRegime(
            regime=regime, asset_class_outlook=asset_class_outlook,
            favored_symbols=favored, avoid_symbols=avoid,
    ...
    def air_agent_generate_signals(self, symbol: str, current_price: float,
                                   navagraha: NavagrahaState) -> AirSignal:
        prices = list(self.price_history.get(symbol, []))
        
        if len(prices) < 20:
            return AirSignal(
                symbol=symbol, action="HOLD", confidence=0.3,
                entry_price=current_price, stop_loss=current_price * 0.95,
                take_profit=current_price * 1.10,
                technical_summary="Onvoldoende data",
                indicators={}, air_dharma="Vayu wacht"
            )
        ...
        # V5: Apply symbol-specific R:R overrides
        risk_override = SYMBOL_RISK_OVERRIDES.get(symbol, {})
        sl_mult = risk_override.get("stop_loss_mult", 2.0)
        tp_mult = risk_override.get("take_profit_mult", 3.0)
        ...
        indicators = {
            "rsi": round(rsi, 2), "ema_20": round(ema_20, 2),
            "ema_50": round(ema_50, 2), "volatility_pct": round(volatility, 2),
            "trend": "bullish" if trend_bullish else "bearish" if trend_bearish else "neutral"
        }
        
        return AirSignal(
            symbol=symbol, action=action, confidence=confidence,
            entry_price=current_price, stop_loss=round(stop_loss, 2),
    ...
    def earth_agent_valuate(self, symbol: str, current_price: float,
                           market_regime: str) -> EarthValuation:
        prices = list(self.price_history.get(symbol, []))
        
        if len(prices) < 30:
            return EarthValuation(
                symbol=symbol, fair_value=current_price,
                current_price=current_price, valuation_gap_pct=0,
                verdict="FAIR", confidence=0.4,
        ...
        return EarthValuation(
            symbol=symbol, fair_value=round(fair_value, 2),
            current_price=current_price, valuation_gap_pct=round(valuation_gap, 2),
            verdict=verdict, confidence=confidence,
            methodology=methodology, earth_dharma=dharma
        )
    
    # ============ ETHER AGENT V5 ============
    
    def ether_agent_synthesize(self, fire: FireDecision, water: WaterRegime,
                               air: AirSignal, earth: EarthValuation,
                               navagraha: NavagrahaState,
                               portfolio_value: float) -> EtherSynthesis:
        """V5: Lower Saturn threshold for 22-28% execute rate"""
        ...
        harmony_score = sum(
            confidences[agent] * weights[agent]
            for agent in confidences
        )
        
        # V5: Use planet-specific threshold (Saturn 0.42!)
        execute_threshold = HARMONY_THRESHOLDS.get(
            navagraha.dominant_planet, 
            HARMONY_THRESHOLDS["DEFAULT"]
        )
        
        # V5: Easier consensus (2+ agents)
        agents_above_baseline = sum(
            1 for agent, conf in confidences.items()
            if conf > 0.50
        )
        consensus_achieved = agents_above_baseline >= 2
        
        if consensus_achieved:
            harmony_score = min(1.0, harmony_score * 1.15)
            self.consensus_count += 1
        
        # V5: Check against LOWER planet-specific threshold
        if harmony_score < execute_threshold:
            return EtherSynthesis(
                final_decision="BLOCK", harmony_score=harmony_score,
                approved_symbol=None, approved_action=None, approved_qty=0,
                approved_price=0, stop_loss=0, take_profit=0,
                execution_urgency="none", consensus_achieved=consensus_achieved,
        ...
        if fire.decision == "BLOCK":
            return EtherSynthesis(
                final_decision="BLOCK", harmony_score=harmony_score,
                approved_symbol=air.symbol, approved_action=None, approved_qty=0,
                approved_price=0, stop_loss=0, take_profit=0,
                execution_urgency="none", consensus_achieved=consensus_achieved,
                blocking_agent="fire",
        ...
        # V5: Increase position size for crypto micro-symbols
        if air.symbol in CRYPTO_MICRO_SYMBOLS:
            position_factor *= 1.5
        ...
        # V5: Apply position factor to crypto micro symbols
        ...
        return EtherSynthesis(
            final_decision=final_decision, harmony_score=harmony_score,
            approved_symbol=air.symbol,
            approved_action=air.action if final_decision == "EXECUTE" else "HOLD",
            approved_qty=approved_qty, approved_price=air.entry_price,
            stop_loss=air.stop_loss, take_profit=air.take_profit,
            execution_urgency=execution_urgency, consensus_achieved=consensus_achieved,
            blocking_agent=None,
    ...
    def process_trading_cycle(self, symbol: str, current_price: float,
        ...
        if not self.is_symbol_allowed(symbol):
            return EtherSynthesis(
                final_decision="BLOCK", harmony_score=0.0,
                approved_symbol=symbol, approved_action=None, approved_qty=0,
                approved_price=0, stop_loss=0, take_profit=0,
                execution_urgency="none", consensus_achieved=False,
                blocking_agent="fire",
        ...
        # V5: Even higher base harmony with lower thresholds
        ...
        fire = self.fire_agent_assess(
            symbol=symbol, proposed_action=air.action, proposed_qty=proposed_qty,
            price=current_price, portfolio_value=portfolio_value, navagraha=navagraha,
            harmony_score=base_harmony, prana_level=prana_level,
            market_regime=water.regime, volatility_24h=volatility
        )
        
        ether = self.ether_agent_synthesize(
            fire=fire, water=water, air=air, earth=earth,
            navagraha=navagraha, portfolio_value=portfolio_value
        )
    ...
    def get_agent_stats(self) -> Dict[str, Any]:
        ...
        stats["symbol_risk_overrides"] = list(SYMBOL_RISK_OVERRIDES.keys())
        return stats
