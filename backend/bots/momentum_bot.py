...
class MomentumBot(BaseTradingBot):
    ...
    async def analyze_market(self, symbol: str, price_data: list[float]) -> dict[str, Any]:
        ...
        roc_3 = (
            ((current_price - price_data[-4]) / price_data[-4]) * 100 if len(price_data) >= 4 else 0
        )
        roc_5 = (
            ((current_price - price_data[-6]) / price_data[-6]) * 100 if len(price_data) >= 6 else 0
        )
        roc_10 = (
            ((current_price - price_data[-11]) / price_data[-11]) * 100
        ...
        acceleration = (
            roc_3 - (price_data[-2] - price_data[-5]) / price_data[-5] * 100
            if len(price_data) >= 5
            else 0
        )
        ...
        recent_volatility = (
            sum(abs(price_data[i] - price_data[i - 1]) / price_data[i - 1] for i in range(-5, 0))
            / 5
            * 100
        )
