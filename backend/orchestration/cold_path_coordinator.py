...
class ColdPathCoordinator:
    ...
    def __init__(
        ...
        self.update_interval = max(
            self.MIN_UPDATE_INTERVAL, min(update_interval, self.MAX_UPDATE_INTERVAL)
        )
    ...
    def make_decision(self) -> CoordinatorDecision:
        ...
        try:
            ...
            with self.write_lock:
                ...
                if (
                    self.last_best_decision is None
                    or decision.confidence > self.last_best_decision.confidence
                ):
        ...
        with self.agent_lock:
            for agent in self.agents:
                ...
                try:
                    ...
                    logger.debug(
                        f"Agent {agent_name}: action={output.get('action')}, "
    ...
    def write_config(self, decision: Optional[CoordinatorDecision] = None) -> None:
        ...
        try:
            with self.write_lock:
                ...
                logger.info(
                    f"Wrote config: action={decision.action}, "
    ...
    def set_update_interval(self, interval: float) -> None:
        ...
        """
        self.update_interval = max(
            self.MIN_UPDATE_INTERVAL, min(interval, self.MAX_UPDATE_INTERVAL)
        )
