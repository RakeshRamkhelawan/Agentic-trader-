...
class Phase11IntegrationCoordinator:
    ...
    def execute_agents(self) -> Dict[str, Dict[str, Any]]:
        ...
        for name, agent in self.agents.items():
            try:
                ...
                logger.debug(f"{name} decision: action={decision['action']}, confidence={decision['confidence']}")
    ...
    def aggregate_decisions(self, decisions: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        ...
        """
        valid_decisions = {
            name: dec for name, dec in decisions.items() 
            if dec is not None
        }
        ...
        total_weight = sum(
            self.agent_weights[name] for name in valid_decisions.keys()
        )
