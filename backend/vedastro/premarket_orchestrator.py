...
from .enhanced_orchestrator import EnhancedAstroOrchestrator, CompleteAstroAnalysis
from .advanced_features import PanchaPakshiData, MuhurthaData
...
class VedAstroPreMarketOrchestrator:
    ...
    async def generate_daily_plan(self, symbols: List[str], 
                                  analysis_date: Optional[datetime] = None) -> DailyTradingPlan:
        """
        ...
        logger.info(f"Daily plan generated: {len(plan.top_assets)} top assets, "
                   f"{len(plan.trading_windows)} trading windows")
        
        return plan
    
    async def _analyze_all_assets(self, symbols: List[str], 
                                  date: datetime) -> Dict[str, CompleteAstroAnalysis]:
    ...
    def _score_assets(self, analyses: Dict[str, CompleteAstroAnalysis]) -> List[AssetScore]:
        ...
        for symbol, analysis in analyses.items():
            score = AssetScore(
                ...
                pancha_pakshi_strength=analysis.pancha_pakshi.strength if analysis.pancha_pakshi else 0.5,
    ...
    def _build_trading_plan(self, date: str, asset_scores: List[AssetScore],
                           muhurtha: Optional[MuhurthaData],
                           time_windows: List[TimeWindow]) -> DailyTradingPlan:
        ...
        # Assets to avoid
        avoid_assets = [a.symbol for a in asset_scores if a.overall_score < self.MIN_SCORE_THRESHOLD]
        
        # Split windows
        trading_windows = [w for w in time_windows if w.window_type in 
                          [TradingWindow.PRIME, TradingWindow.GOOD, TradingWindow.MODERATE]]
        blocked_windows = [w for w in time_windows if w.window_type in 
                          [TradingWindow.AVOID, TradingWindow.BLOCK]]
        
        # Best entry/exit times
        best_entry_times = [w.start_time for w in time_windows 
                           if w.window_type == TradingWindow.PRIME]
        best_exit_times = [w.start_time for w in time_windows 
    ...
    def is_favorable_day(self) -> bool:
        ...
        if not self.daily_plan:
            return False
        return self.daily_plan.is_favorable_day and self.daily_plan.muhurtha_rating >= self.MIN_MUHURTHA_RATING
...
def can_trade_symbol(symbol: str, plan: DailyTradingPlan, 
...
if __name__ == "__main__":
    ...
    async def demo():
        ...
        for window in plan.trading_windows:
            ...
            print(f"  {emoji} {window.start_time.strftime('%H:%M')}-{window.end_time.strftime('%H:%M')}: "
                  f"{window.description}")
        
        print("\nBlocked Windows:")
        for window in plan.blocked_windows:
            print(f"  🔴 {window.start_time.strftime('%H:%M')}-{window.end_time.strftime('%H:%M')}: "
                  f"{window.description}")
