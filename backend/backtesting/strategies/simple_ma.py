...
class MovingAverageStrategy(Strategy):
    ...
    def __init__(self, exchange, short_window: int = 10, long_window: int = 30,
        ...
        """
        super().__init__(exchange, position_sizer=position_sizer, 
                        slippage_model=slippage_model, fill_model=fill_model)
    ...
    async def on_start(self):
        """Initialize strategy."""
        print(f"MA Strategy Started: short_window={self.short_window}, long_window={self.long_window}")
    ...
    async def on_bar(self, symbol: str, bar: Dict[str, Any]):
        ...
        if short_ma > long_ma and current_qty == 0:
            ...
            self.trades_count += 1
            print(f"BUY {symbol}: {position_size:.4f} @ {close_price} "
        ...
        # Death Cross (Sell Signal)
            ...
            self.trades_count += 1
            print(f"SELL {symbol}: {current_qty:.4f} @ {close_price} "
                  f"(short_ma={short_ma:.2f} < long_ma={long_ma:.2f})")
        
        # Update portfolio value for position sizing
        if self.exchange.cash >= 0:
            self.update_portfolio_value(self.exchange.cash + sum(
