...
class SimulatedExchange:
    ...
    def execute_market_order(self, symbol: str, side: OrderSide, quantity: float, current_price: float, timestamp: datetime) -> Trade:
        ...
        if side == OrderSide.BUY:
            ...
            if not current_pos or current_pos.quantity < quantity:
                raise ValueError(f"Insufficient shares: Have {current_pos.quantity if current_pos else 0}, need {quantity}")
