...
if PROMETHEUS_AVAILABLE:
    ...
    TATTVA_MODE_GAUGE = Gauge(
        'tattva_current_mode',
        'Current Tattva traversal mode (0=sparse, 1=full)'
    )
else:
...
class SystemIdentityOptimized:
        ...
        try:
            ...
            if self.enable_metrics and TATTVA_TRAVERSAL_LATENCY:
                TATTVA_TRAVERSAL_LATENCY.labels(
                    mode=mode.value,
                    layer_count=len(layers_to_traverse)
                ).observe(latency_us)
    ...
    def get_performance_statistics(self) -> Dict[str, Any]:
        ...
        # Separate by mode
        sparse_latencies = [
            lat for lat, trav in zip(latencies, self.performance_history.get("tattva_traversals", []))
        ...
        return {
            ...
            "sparse_mode": {
                ...
                "target_met": all(lat < 80 for lat in sparse_latencies) if sparse_latencies else True,
            },
            "full_mode": {
