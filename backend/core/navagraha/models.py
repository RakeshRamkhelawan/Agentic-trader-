...
class PlanetState(BaseModel):
    ...
    longitude: float = Field(..., ge=0.0, lt=360.0, description="Ecliptic longitude in degrees [0, 360)")
    latitude: float = Field(..., ge=-90.0, le=90.0, description="Ecliptic latitude in degrees [-90, 90]")
    ...
    distance_au: Optional[float] = Field(None, ge=0.0, description="Distance from Earth in Astronomical Units")
...
class GunaDistribution(BaseModel):
    sattva: float = Field(..., ge=0.0, le=1.0, description="Sattva (harmony, wisdom) proportion [0, 1]")
    rajas: float = Field(..., ge=0.0, le=1.0, description="Rajas (activity, passion) proportion [0, 1]")
    tamas: float = Field(..., ge=0.0, le=1.0, description="Tamas (inertia, darkness) proportion [0, 1]")
    calculated_at: datetime = Field(..., description="UTC timestamp of calculation")
    ...
    def balance_score(self) -> float:
        ideal = 1.0 / 3.0
        deviation = (
            abs(self.sattva - ideal) +
            abs(self.rajas - ideal) +
            abs(self.tamas - ideal)
        )
...
class NavagrahaState(BaseModel):
    ...
    aspects: List[PlanetaryAspect] = Field(default_factory=list, description="Active planetary aspects")
    rahu_kala_active: bool = Field(..., description="True if currently in Rahu Kala period")
    current_dasha: Optional[PlanetName] = Field(None, description="Current Mahadasha period lord")
    ...
    @classmethod
    def validate_nine_planets(cls, v: Dict[PlanetName, PlanetState]) -> Dict[PlanetName, PlanetState]:
        if len(v) != 9:
        ...
        if required != present:
            ...
            raise ValueError(
                f"Invalid planet set. Missing: {missing}, Extra: {extra}"
            )
