...
class MCTSNode:
    ...
    def uct_score(self, exploration_constant: float = 1.414) -> float:
        ...
        exploration = (
            exploration_constant * np.sqrt(np.log(self.parent.visits) / self.visits)
            if self.parent
            else 0
        )
...
class StrategicMCTSPlanner:
    ...
    def plan(
        ...
        for i in range(self.simulations):
            ...
            if i % 20 == 0:
                print(
                    f"  MCTS Sim {i}/{self.simulations}: Best value {root.value/max(1,root.visits):.3f}"
                )
    ...
    def _generate_actions(self, state: Dict, symbols: List[str]) -> List[Dict]:
        ...
        for symbol in symbols:
            ...
            if portfolio.get("positions", {}).get(symbol):
                actions.append({"type": "sell", "symbol": symbol, "reason": "strategic_exit"})
