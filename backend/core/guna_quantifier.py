...
class GunaQuantifier:
    ...
    def quantify_text(self, text: str) -> GunaVector:
        ...
        # Rajas indicatoren
        rajas_score = 0
        rajas_keywords = ["surges", "jumps", "rises", "falls", "breakout", "action", "volatility", "movement", "change", "momentum", "buy", "sell", "urgent", "breaking"]
        for kw in rajas_keywords:
            rajas_score += text_lower.count(kw)
            
        # Tamas indicatoren
        tamas_score = 0
        tamas_keywords = ["crash", "fear", "panic", "collapse", "stagnant", "bear", "crisis", "tension", "uncertainty", "frozen", "resist", "chaos"]
        for kw in tamas_keywords:
            tamas_score += text_lower.count(kw)
            
        # Sattva indicatoren (als tegenhanger of neutraal)
        sattva_score = 0
        sattva_keywords = ["stable", "growth", "predictable", "healthy", "balanced", "calm", "objective", "factual", "clear"]
