...
class MemorySystem:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        if not self.memory_buffer:
            return {'total_memories': 0, 'total_clusters': 0, 'avg_cluster_size': 0, 'win_rate': 0.0}
        outcomes = [m.outcome for m in self.memory_buffer]
        return {
            'total_memories': len(self.memory_buffer),
            'total_clusters': len(self.clusters),
            'avg_cluster_size': len(self.memory_buffer) / len(self.clusters) if self.clusters else 0,
