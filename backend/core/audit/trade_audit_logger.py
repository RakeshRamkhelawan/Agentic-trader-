...
class TradeAuditLogger:
    ...
    def log_collective_decision(
        ...
        for sig in agent_signals:
            signal_logs.append(AgentDecisionLog(
                timestamp=datetime.now().isoformat(),
                agent_name=sig.agent_name,
                agent_element=sig.element.value if hasattr(sig.element, 'value') else str(sig.element),
                symbol=getattr(market_state, 'symbol', 'unknown'),
        ...
        log = CollectiveDecisionLog(
            ...
            final_action=decision.action.name if hasattr(decision.action, 'name') else str(decision.action),
            ...
            collective_guna=getattr(decision.guna_state, '__dict__', {}) if hasattr(decision, 'guna_state') else {},
            guna_dominant=getattr(decision.guna_state, 'dominant', lambda: 'unknown')() if hasattr(decision, 'guna_state') else 'unknown',
            guna_purity=getattr(decision.guna_state, 'purity_index', lambda: 0.0)() if hasattr(decision, 'guna_state') else 0.0,
    ...
    def log_risk_check(
        ...
        log = RiskCheckLog(
            ...
            position_limit_check=len(getattr(risk_manager, 'active_positions', {})) < getattr(risk_manager, 'max_total', 5),
