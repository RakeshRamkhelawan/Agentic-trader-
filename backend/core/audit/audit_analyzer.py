...
class AuditAnalyzer:
    ...
    def generate_report(self) -> str:
        ...
        for agent, stats in sorted(agent_stats.items()):
            lines.append(f"  {agent:20s} | Signals: {stats['count']:5d} | "
        ...
        lines.append(f"  Maya Detections:        {coll_stats['maya_count']} ({coll_stats['maya_rate']:.1%})")
        ...
        lines.append(f"  Passed:                 {risk_stats['passed']} ({risk_stats['pass_rate']:.1%})")
        lines.append(f"  Rejected:               {risk_stats['rejected']} ({risk_stats['reject_rate']:.1%})")
        ...
        lines.append(f"  Symbol Distribution:")
    ...
    def _analyze_agents(self) -> Dict[str, Dict]:
        ...
        for agent, data in stats.items():
            result[agent] = {
                ...
                'dominant_guna': max(data['guna_counts'], key=data['guna_counts'].get) if data['guna_counts'] else 'unknown'
            }
