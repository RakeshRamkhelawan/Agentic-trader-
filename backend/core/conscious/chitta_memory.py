...
class ChittaMemory:
    ...
    def __init__(self, storage_path: str = "backend/data/conscious_memory"):
        ...
        print(f"[CHITTA] Memory initialized: {len(self.trades)} trades, {len(self.strategies)} strategies")
    ...
    def get_top_strategies(self, n: int = 3) -> List[StrategyPerformance]:
        ...
        scored = []
        for s in active:
            score = s.avg_return / (s.max_drawdown + 0.01) if s.max_drawdown > 0 else s.avg_return * 10
    ...
    def should_pause_trading(self, drawdown_limit: float = 0.08) -> Tuple[bool, str]:
        ...
        if self.session_stats['current_drawdown'] > drawdown_limit:
            return True, f"Drawdown {self.session_stats['current_drawdown']:.1%} > {drawdown_limit:.1%}"
