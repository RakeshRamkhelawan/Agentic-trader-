...
"""
    ...
    def _update_emotional_state(self):
        ...
        self.state.confidence_level = min(1.0,
            0.3 +  # Base confidence
            self.state.win_streak * 0.15 +  # Each win adds 15%
            (self.state.total_pnl / 10000) * 0.1  # PnL contribution
        )
        
        # Clarity based on harmony and inverse anxiety
        self.state.clarity_level = (
            self.state.avg_harmony_recent * 0.7 +
            (1 - self.state.anxiety_level) * 0.3
        )
...
"""
    ...
    def decide_action(self, 
                      ...
                      memory_insights: Dict = None) -> Dict[str, Any]:
        """
        ...
        return {
            'action': decision_action.name if hasattr(decision_action, 'name') else str(decision_action),
