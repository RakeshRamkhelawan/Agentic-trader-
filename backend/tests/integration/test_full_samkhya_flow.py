...
def mock_guna_quantifier():
    ...
    quantifier.quantify_text.return_value = GunaVector(sattva=0.3, rajas=0.5, tamas=0.2) # Standaard Rajas
    quantifier.quantify_numerical_data.return_value = GunaVector(sattva=0.4, rajas=0.4, tamas=0.2) # Standaard neutraal voor nummer
    return quantifier
...
def trading_system_samkhya(
    ...
    orchestrator.agents["risk_guardian_v1"] = risk_validator # Risk direct, zonder message_bus (voor nu)
    ...
    orchestrator.agents["execution_gateway_v1"].submit_order = AsyncMock(side_effect=shadow_portfolio.submit_order)
    orchestrator.agents["execution_gateway_v1"].get_balance = AsyncMock(side_effect=shadow_portfolio.get_balance)
...
@pytest.mark.asyncio
async def test_samkhya_e2e_rajasic_news_triggers_trade(trading_system_samkhya, mock_httpx_async_client):
    """
    Scenario:
    ...
    assert sys["orchestrator"].current_guna_balance == GunaVector(sattva=1/3, rajas=1/3, tamas=1/3)
    ...
    mock_httpx_async_client.return_value.get.return_value = MagicMock(status_code=200, text="<html><body><p>"+rajasic_news_text+"</p></body></html>")

    # Mock ResearchAgent's interne analyse (om GunaQuantifier te testen)
    with patch('backend.services.research_agent.analyze_text_with_llm', new_callable=AsyncMock) as mock_analyze:
        ...
        await sys["orchestrator"].handle_message(AgentMessage(source="orchestrator", target="research_v1", type="TIMER_TICK_1MIN", payload={"text": rajasic_news_text}))
    ...
    assert sys["orchestrator"].current_guna_balance.rajas > sys["orchestrator"].current_guna_balance.sattva
    assert sys["orchestrator"].current_guna_balance.rajas > sys["orchestrator"].current_guna_balance.tamas
    ...
    sys["risk_validator"].validate_order(order_request_from_orchestrator, current_price=current_price)
    
    # Voer de order uit via de gateway
    await sys["orchestrator"].agents["execution_gateway_v1"].submit_order(order_request_from_orchestrator)
...
@pytest.mark.asyncio
async def test_samkhya_e2e_tamasic_market_blocks_trade(trading_system_samkhya, mock_httpx_async_client):
    """
    Scenario:
