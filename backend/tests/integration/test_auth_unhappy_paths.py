...
def test_invalid_authorization_header_format(mock_middleware_validator):
    response = client.get(
        "/api/v1/trading/orders/active", headers={"Authorization": "Basic somebase64"}
    )
...
def test_valid_token_access(mock_middleware_validator):
    ...
    mock_middleware_validator.return_value = TokenPayload(
        sub="user123", tenant_id="tenant-A", roles=["user"], exp=9999999999
    )
    ...
    with patch(
        "backend.services.trading_service.TradingService.get_active_orders",
        return_value=[],
    ):
        response = client.get(
            "/api/v1/trading/orders/active",
