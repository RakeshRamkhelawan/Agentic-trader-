...
async def test_sentiment_agent_analyzes_market_tick():
    ...
    features = {"price": 50000.0, "volume": 1.5, "price_change": 500.0}
...
async def test_sentiment_agent_publishes_to_event_bus():
    ...
    await agent.publish_thought(
        reasoning="Market shows bearish signals", confidence=0.75, data=result
    )
...
async def test_sentiment_agent_handles_multiple_ticks():
    ...
    agent = SentimentAgent(agent_name="multi_tick_agent", llm_provider=llm)
...
async def test_sentiment_agent_maintains_state_across_analyses():
    ...
    llm = MockLLMProvider()
    agent = SentimentAgent(agent_name="stateful_agent", llm_provider=llm)
...
async def test_sentiment_agent_with_real_event_bus_mock():
    ...
    context = {"symbol": "BTC/USD", "timestamp": datetime.now(UTC)}

    result = await agent.analyze(features, context)
    await agent.publish_thought(
        reasoning="Strong bullish momentum detected", confidence=0.88, data=result
    )
...
async def test_sentiment_agent_error_handling_with_llm_failure():
    ...
    llm = FailingLLM()
    agent = SentimentAgent(agent_name="error_handling_agent", llm_provider=llm)
...
async def test_sentiment_agent_reasoning_chain_tracking():
    ...
    llm = MockLLMProvider()
    agent = SentimentAgent(agent_name="reasoning_agent", llm_provider=llm)
...
async def test_sentiment_agent_health_monitoring():
    ...
    llm = MockLLMProvider()
    agent = SentimentAgent(agent_name="health_monitor_agent", llm_provider=llm)
...
async def test_sentiment_agent_context_preservation():
    ...
    llm = MockLLMProvider()
    agent = SentimentAgent(agent_name="context_agent", llm_provider=llm)
