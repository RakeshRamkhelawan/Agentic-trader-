...
class TestWiringEndToEndIntegration:
    ...
    async def test_complete_user_journey(self, async_client: AsyncClient, unique_email: str):
        ...
        assert settings_response.status_code == 200, f"Get settings failed: {settings_response.text}"
        ...
        assert profile_response.status_code == 200, f"Update profile failed: {profile_response.text}"
        ...
        assert notifications_response.status_code == 200, f"Update notifications failed: {notifications_response.text}"
        ...
        assert preferences_response.status_code == 200, f"Update preferences failed: {preferences_response.text}"
        ...
        assert tournaments_response.status_code == 200, f"Get tournaments failed: {tournaments_response.text}"
        ...
        print(f"       - Available badges: {badges_data.get('total', len(badges_data.get('badges', [])))}")
