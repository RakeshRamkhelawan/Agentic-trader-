...
async def test_clickhouse_client_stores_market_tick():
    ...
    with patch("backend.storage.clickhouse_client.clickhouse_connect.get_async_client", return_value=mock_client):
...
async def test_event_bus_to_clickhouse_pipeline():
    ...
    with patch("redis.asyncio.from_url", return_value=mock_redis):
        with patch("backend.storage.clickhouse_client.clickhouse_connect.get_async_client", return_value=mock_ch_client):
            # Setup components
...
async def test_bulk_event_storage():
    ...
    with patch("backend.storage.clickhouse_client.clickhouse_connect.get_async_client", return_value=mock_client):
        ...
        assert call_args[0][1] == events or call_args[1].get("data") == events or len(call_args[0]) >= 2
...
async def test_concurrent_event_storage():
    ...
    with patch("backend.storage.clickhouse_client.clickhouse_connect.get_async_client", return_value=mock_client):
        ...
        tasks = [
            ch_client.insert("market_ticks", batch)
