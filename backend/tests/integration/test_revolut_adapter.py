...
@pytest.fixture
def mock_adapter():
    with patch("backend.execution.exchange_adapter.serialization.load_pem_private_key") as mock_load:
...
async def test_get_instruments_success(mock_adapter):
    ...
    mock_data = {
         "pairs": [
            {"symbol": "BTC-EUR", "base_currency": "BTC", "quote_currency": "EUR", "min_order_size": "0.0001", "max_order_size": "10.0"},
            {"symbol": "ETH-EUR", "base_currency": "ETH", "quote_currency": "EUR", "min_order_size": "0.01", "max_order_size": "100.0"}
        ]
    }
    # Mock the response
...
async def test_get_candles_success(mock_adapter):
    ...
    mock_data = {"data": [
        {"t": 1600000000, "o": 100, "h": 110, "l": 90, "c": 105, "v": 1000}
    ]}
