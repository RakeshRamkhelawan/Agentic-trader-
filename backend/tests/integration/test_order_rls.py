...
async def test_14_rls_orders_isolation(system_db):
    ...
    async with SessionManager.tenant_session(tenant_a) as session_a:
        # Should see A
        res_a = await session_a.execute(text(f"SELECT COUNT(*) FROM orders WHERE id = '{order_a_id}'"))
        assert res_a.scalar() == 1
        
        # Should NOT see B
        res_b = await session_a.execute(text(f"SELECT COUNT(*) FROM orders WHERE id = '{order_b_id}'"))
...
async def test_15_system_admin_orders_access(system_db):
    ...
    async with SessionManager.system_admin_session() as admin_session:
        res = await admin_session.execute(text(f"SELECT COUNT(*) FROM orders WHERE id = '{order_c_id}'"))
        assert res.scalar() == 1
