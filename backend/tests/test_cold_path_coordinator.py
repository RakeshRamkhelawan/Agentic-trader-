...
class TestColdPathCoordinatorOrchestration:
    ...
    def test_execute_all_agents(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            agent1.analyze.return_value = {'action': 1, 'confidence': 0.8, 'reasoning': 'bullish'}
            
            agent2 = MagicMock()
            agent2.name = "regime"
            agent2.analyze.return_value = {'action': 1, 'confidence': 0.6, 'reasoning': 'uptrend'}
    ...
    def test_handles_conflicting_decisions(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            agent2 = MagicMock()
            agent2.name = "regime"
            agent2.analyze.return_value = {'action': 2, 'confidence': 0.6, 'reasoning': 'bearish'}
    ...
    def test_uses_agent_weights(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            agent1 = MagicMock()
            agent1.name = "sentiment"
            agent1.analyze.return_value = {'action': 1, 'confidence': 0.5, 'reasoning': 'slightly bullish'}
            coordinator.register_agent(agent1, weight=1.0)
            
            agent2 = MagicMock()
            agent2.name = "regime"
            agent2.analyze.return_value = {'action': 2, 'confidence': 0.5, 'reasoning': 'slightly bearish'}
...
class TestColdPathConfigUpdates:
    ...
    def test_writes_decision_to_fastconfig(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            agent = MagicMock()
            agent.name = "sentiment"
            agent.analyze.return_value = {'action': 1, 'confidence': 0.85, 'reasoning': 'bullish'}
            coordinator.register_agent(agent)
    ...
    def test_throttles_config_writes(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            agent = MagicMock()
            agent.name = "sentiment"
            agent.analyze.return_value = {'action': 1, 'confidence': 0.8, 'reasoning': 'bullish'}
            coordinator.register_agent(agent)
    ...
    def test_writes_best_decision(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            agent.analyze.return_value = {'action': 1, 'confidence': 0.6, 'reasoning': 'weak bullish'}
            decision1 = coordinator.make_decision()
            
            # Second decision: action=1, confidence=0.8 (higher)
            agent.analyze.return_value = {'action': 1, 'confidence': 0.8, 'reasoning': 'strong bullish'}
            decision2 = coordinator.make_decision()
