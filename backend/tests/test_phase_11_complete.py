...
class TestPhase11AgentMocking:
    ...
    def test_multiple_mock_agents_callable(self, mock_sentiment_agent, mock_market_regime_agent, mock_risk_governor):
        ...
        for agent in [mock_sentiment_agent, mock_market_regime_agent, mock_risk_governor]:
...
class TestPhase11SentimentAgentIntegration:
    ...
    def test_sentiment_agent_registered(self, temp_config_file, mock_sentiment_agent):
        ...
        coord = create_coordinator_with_custom_agents(temp_config_file, sentiment_agent=mock_sentiment_agent)
...
class TestPhase11MarketRegimeAgentIntegration:
    ...
    def test_market_regime_agent_registered(self, temp_config_file, mock_market_regime_agent):
        ...
        coord = create_coordinator_with_custom_agents(temp_config_file, market_regime_agent=mock_market_regime_agent)
    ...
    @pytest.mark.unit
    def test_multiple_agents_aggregation(self, temp_config_file, mock_sentiment_agent, mock_market_regime_agent):
        """Sentiment and MarketRegime agents properly aggregated."""
        coord = create_coordinator_with_custom_agents(
            temp_config_file, sentiment_agent=mock_sentiment_agent, market_regime_agent=mock_market_regime_agent
        )
        decision = coord.make_decision()
    ...
    def test_conflicting_decisions_resolved(self, temp_config_file):
        ...
        coord = create_coordinator_with_custom_agents(temp_config_file, sentiment_agent=sent, market_regime_agent=market)
        decision = coord.make_decision()
...
class TestPhase11RiskGovernorIntegration:
    ...
    def test_risk_governor_registered(self, temp_config_file, mock_risk_governor):
        ...
        coord = create_coordinator_with_custom_agents(temp_config_file, risk_governor=mock_risk_governor)
    ...
    def test_risk_governor_overrides_decisions(self, temp_config_file):
        ...
        coord = create_coordinator_with_custom_agents(temp_config_file, sentiment_agent=sent, risk_governor=risk)
        coord.agent_weights["RiskGovernor"] = 2.0  # Higher weight
        decision = coord.make_decision()
        assert decision['action'] == 0  # Hold due to risk constraint
    
    @pytest.mark.unit
    def test_all_three_agents_aggregation(self, temp_config_file, mock_sentiment_agent, mock_market_regime_agent, mock_risk_governor):
...
class TestPhase11FullE2EPipeline:
    ...
    @pytest.mark.unit
    def test_decision_quality(self, temp_config_file, mock_sentiment_agent, mock_market_regime_agent, mock_risk_governor):
        ...
        decision = coord.make_decision()
        assert decision['action'] in [0, 1, 2] and decision['confidence'] > 0.5 and decision['reasoning']
...
class TestPhase11ConcurrentAgents:
    ...
    @pytest.mark.unit
    def test_decision_isolation(self, temp_config_file, mock_sentiment_agent, mock_market_regime_agent):
...
class TestPhase11ErrorScenarios:
    ...
    def test_all_agents_fail_fallback(self, temp_config_file):
        ...
        for agent in [sent, market, risk]:
            agent.analyze = MagicMock(side_effect=Exception("Failed"))
        coord = create_coordinator_with_custom_agents(
            temp_config_file, sentiment_agent=sent, market_regime_agent=market, risk_governor=risk
        )
        decision = coord.make_decision()
    ...
    def test_agent_recovery(self, temp_config_file):
        ...
        # Second: recover
        sent.analyze = MagicMock(return_value={"action": 1, "confidence": 0.85, "reasoning": "Recovered"})
