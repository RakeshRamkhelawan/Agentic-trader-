...
ADMIN_USER = {"email": "admin@agentic-trader.com", "password": "admin@123"}

DEMO_USER = {"email": "demo@agentic-trader.com", "password": "demo@123"}
...
async def test_login_nonexistent_user_returns_401():
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        response = await client.post(
            "/api/v1/auth/login",
            json={"email": "nonexistent@nowhere.com", "password": "SomePassword123!"},
        )
...
async def test_demo_user_can_login():
    ...
    """
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        response = await client.post("/api/v1/auth/login", json=DEMO_USER)
...
async def test_token_from_login_works_for_protected_route():
    ...
    ), f"Protected route failed: {protected_response.text}"
