...
def test_get_collection_specific_tenant(memory_agent, mock_chroma_client):
    ...
    with patch("backend.core.memory_agent.get_current_tenant_optional", return_value="tenant-abc"):
        collection = memory_agent._get_collection()
        
        mock_chroma_client.get_or_create_collection.assert_called_with(
            name="tenant-abc_test_thoughts",
            metadata={"hnsw:space": "cosine"}
        )
...
def test_store_thought_injects_tenant_id(memory_agent, mock_chroma_client):
    ...
    with patch("backend.core.memory_agent.get_current_tenant_optional", return_value="tenant-xyz"):
        # We need to ensure _get_collection returns the mock
        with patch.object(memory_agent, '_get_collection', return_value=mock_collection) as mock_get_coll:
