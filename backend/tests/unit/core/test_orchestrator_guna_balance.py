...
def mock_agent_registry():
    ...
    registry.profiles = {
        ...
        "research_v1": AgentProfile(
            ...
            system_directive="",
            allowed_tools=[],
            subscriptions=["GUNA_SIGNAL", "NEWS_DATA"],
        ),
...
@pytest.mark.asyncio
async def test_orchestrator_routes_by_guna(
    mock_agent_registry, mock_guna_quantifier, mock_intent_monitor
):
...
@pytest.mark.asyncio
async def test_orchestrator_maintains_guna_balance(
    mock_agent_registry, mock_guna_quantifier, mock_intent_monitor
):
