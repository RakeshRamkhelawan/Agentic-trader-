...
async def test_execute_order_audit(trading_service, mock_audit_logger):
    ...
    with patch.object(trading_service, '_get_exchange_adapter', new_callable=AsyncMock) as mock_get_adapter:
        ...
        # Mock user prefs
        trading_service.settings_service.get_user_preferences = AsyncMock(return_value={"default_exchange": "mock_ex"})
...
async def test_execute_order_audit_failure(trading_service, mock_audit_logger):
    ...
    # Force error
    with patch.object(trading_service, '_get_exchange_adapter', side_effect=ValueError("Exchange Error")):
    ...
    with patch.object(trading_service, '_get_exchange_adapter', new_callable=AsyncMock) as mock_get_adapter:
        ...
        result = await trading_service.execute_order(
            db=mock_db, 
            tenant_id=tenant_id, 
            order_request={"symbol": "BTC", "qty": 1.0, "side": "BUY", "order_type": "MARKET"}, 
            bypass_risk=True
        )
