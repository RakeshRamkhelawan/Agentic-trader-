...
def reset_modules():
    ...
    modules_to_remove = [m for m in sys.modules if 'cognitive_orchestrator' in m or 'telemetry' in m]
        ...
        for collector in collectors:
            try:
                if hasattr(collector, '_name'):
                    if any(x in collector._name for x in ['guna', 'trading', 'order', 'orchestrator']):
