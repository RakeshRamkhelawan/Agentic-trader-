...
async def test_sell_stock_increases_cash(manager):
    ...
    manager.update_price("AAPL", 150.0)
    # Eerst kopen
    await manager.submit_order(OrderRequest(symbol="AAPL", qty=10, side=OrderSide.BUY, order_type=OrderType.MARKET))
    
    # Prijs stijgt
    manager.update_price("AAPL", 160.0)
    
    # Verkopen
    await manager.submit_order(OrderRequest(symbol="AAPL", qty=5, side=OrderSide.SELL, order_type=OrderType.MARKET))
