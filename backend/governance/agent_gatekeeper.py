...
class AgentGatekeeper:
    ...
    def __init__(self, override_permissions: Optional[Dict[AgentRole, Set[ToolPermission]]] = None):
        """
    ...
    def authorize(self, agent_name: str, agent_role: AgentRole, required_permission: ToolPermission) -> bool:
        """
        ...
        if required_permission in allowed_permissions:
            logger.debug(f"Authorization GRANTED: {agent_name} ({agent_role}) -> {required_permission}")
            return True
        
        logger.warning(f"Authorization DENIED: {agent_name} ({agent_role}) lacks {required_permission}")
        return False

    def require_permission(self, agent_name: str, agent_role: AgentRole, required_permission: ToolPermission):
        """
