...
def extract_phases_detailed(content):
    ...
    for match in matches:
        ...
        tasks = re.findall(r'###\s*(?:Task|TASK)\s*([\d\.]+)[:\s]*([^\n]+)', phase_content, re.IGNORECASE)
        microtasks = re.findall(r'####\s*(?:Microtask|MICROTASK)\s*([\d\.]+)[:\s]*([^\n]+)', phase_content, re.IGNORECASE)
        
        dependencies = re.findall(r'(?:depends on|requires|after|blocked by)[:\s]*([^\n]+)', phase_content, re.IGNORECASE)
        
        effort = re.search(r'(?:effort|estimate|time)[:\s]*(\d+)\s*(?:hours|days|weeks)', phase_content, re.IGNORECASE)
...
def analyze_api_detailed():
    ...
    for filename in key_files:
        ...
        routes = re.findall(r'@(?:router|app)\.(get|post|put|delete|websocket|patch)\(["\']([^"\']+)["\']', content, re.IGNORECASE)
...
def analyze_navagraha_integration():
    ...
    for py_file in backend_dir.rglob("*.py"):
        ...
        if any(keyword in content.lower() for keyword in ['navagraha', 'ephemeris', 'kerykeion', 'pyswisseph', 'rahu', 'kala']):
...
def main():
    ...
    fase_01 = BASE_DIR / "docs" / "kanban" / "FASE_01_CONSCIOUSNESS_OODA_NAVAGRAHA_BRIDGE.md"
    ...
    if master_kanban.exists():
        ...
        for phase_id, data in phases.items():
            ...
            print(f"  Tasks: {len(data['tasks'])}, Microtasks: {data['microtask_count']}, Effort: {data['effort']}")
    ...
    for item in navagraha_files[:10]:
        print(f"{item['file']}:")
        print(f"  Cache: {'Yes' if item['has_cache'] else 'No'}, WebSocket: {'Yes' if item['has_websocket'] else 'No'}")
        if item['classes']:
    ...
    if handover.exists():
        ...
        architecture_patterns = re.findall(r'(?:OODA|Navagraha|Guna|Tattva|Element|Agent)', content, re.IGNORECASE)
    ...
    if epic_01.exists():
        ...
        findings = re.findall(r'(?:TODO|FIXME|WARNING|ERROR|BUG|ISSUE)[:\s]*([^\n]+)', content, re.IGNORECASE)
        print(f"Code Review Items: {len(findings)}")
        for finding in findings[:5]:
            print(f"  • {finding[:80]}")
        
        recommendations = re.findall(r'(?:recommend|suggest|should|must)[:\s]*([^\n]+)', content, re.IGNORECASE)
