...
# Import V8 Manager
from backend.agents.elemental_agent_manager_v8 import (ElementalAgentManagerV8)
...
class V8BacktestEngine:
    ...
    def run_backtest(self) -> V8BacktestResult:
        ...
        for trading_date in trading_dates:
            ...
            if cycle_count % 500 == 0:
                ...
                logger.info(
                    f"Processed {cycle_count} cycles | Portfolio: ${portfolio_val:,.2f}"
                )
    ...
    def _close_position(
        self, symbol: str, exit_price: float, exit_date: datetime, planet: str
    ):
    ...
    def _generate_result(self) -> V8BacktestResult:
        ...
        avg_trade_pnl = (
            sum(t.realized_pnl or 0 for t in self.trades) / total_trades
            if total_trades > 0
            else 0
        )
