...
from scripts.backtest_with_detailed_logging import (AgentDecision,
                                                    DetailedBacktestLogger,
                                                    LogLevel, TradeExecution)
...
class DetailedBacktestRunner:
    ...
    def get_vedic_context(self, timestamp: datetime) -> Dict:
        ...
        return {
            ...
            "navagraha_dominant": random.choice(
    ...
    def generate_agent_decisions(
        self, symbol: str, candle: Dict, prev_candle: Optional[Dict], vedic: Dict
    ) -> List[AgentDecision]:
        """
        ...
        if prev_candle:
            price_change = (candle["close"] - prev_candle["close"]) / prev_candle[
                "close"
            ]
        else:
            price_change = 0
        ...
        decisions.append(
            AgentDecision(
                ...
                risk_assessment="Normal volatility regime"
                if abs(price_change) < 0.05
                else "Elevated volatility - reduce size",
        ...
        decisions.append(
            AgentDecision(
                ...
                technical_indicators={
                    "portfolio_var": abs(portfolio_value - self.initial_capital)
                ...
                sentiment_score=0.0,
                primary_motivation="Capital preservation"
                if risk_decision in ["BLOCK", "REDUCE"]
    ...
    def execute_trade(
        self, symbol: str, decision: AgentDecision, price: float
        ...
        return TradeExecution(
            ...
            price=price,
            value=value,
            account_balance_before=self.cash
            + (value if decision.decision == "SELL" else 0),
        ...
        for i in range(num_days):
            ...
            for symbol, data in all_data.items():
                ...
                decisions = self.generate_agent_decisions(
                    symbol, candle, prev_candles.get(symbol), vedic
                )
            ...
            if i % 10 == 0:
                prices = {
                    s: all_data[s][i]["close"] for s in all_data if i < len(all_data[s])
                }
                equity = self.get_portfolio_value(prices)
                self.equity_curve.append(
                    {"timestamp": timestamp.isoformat(), "value": equity}
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run detailed backtest with agent logging"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--end", default="2024-03-31", help="End date (keep short for detailed logs)"
    )
    parser.add_argument(
