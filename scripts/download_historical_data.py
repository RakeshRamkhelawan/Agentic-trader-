...
class HistoricalDataDownloader:
    ...
    def download_asset(self, symbol: str, yf_symbol: str) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            df = ticker.history(
                start=self.start_date, end=self.end_date, interval="1d"  # Daily data
            )
    ...
    def download_all(self, max_assets: Optional[int] = None):
        ...
        assets = (
            list(ASSETS.items())[:max_assets] if max_assets else list(ASSETS.items())
        )
...
def main():
    ...
    downloader = HistoricalDataDownloader(start_date=args.start, end_date=args.end)
