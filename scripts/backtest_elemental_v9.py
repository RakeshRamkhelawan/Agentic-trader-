...
logger = logging.getLogger("BacktestV9")

from backend.agents.elemental_agent_manager_v9 import (
    ElementalAgentManagerV9, MacroSignal, IPO_DATES
)
...
class V9BacktestEngine:
    ...
    def __init__(
        ...
        self.agent_manager = ElementalAgentManagerV9()
        ...
        logger.info("=" * 60)
        logger.info("V9: POSITION REVIEW LOOP + SLIPPAGE/COMMISSION")
        logger.info("=" * 60)
    ...
    def run_backtest(self) -> V9BacktestResult:
        ...
        logger.info("\n" + "=" * 60)
        logger.info("STARTING V9 BACKTEST")
        logger.info("=" * 60)
    ...
    def _process_day(self, trading_date: datetime, price_data: Dict[str, List[Dict]]):
        ...
        # V9: Time-based exits first
        self._check_time_based_exits(trading_date, price_data)
        
        # V9: Agent-based position review
        for symbol in list(self.open_positions.keys()):
            ...
            should_exit, exit_reason = self.agent_manager.evaluate_open_position(
                symbol, current_price, macro_signal
            )
    ...
    def _generate_result(self, price_data: Dict) -> V9BacktestResult:
...
def main():
    """Run V9 smoke test (2021)"""
    symbols = ["BTC", "ETH", "SPY", "QQQ", "GLD", "AAPL", "MSFT", "UNI", "SOL"]
    
    engine = V9BacktestEngine(
        symbols=symbols,
    ...
    output_file = f"backtest_v9_2021_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
