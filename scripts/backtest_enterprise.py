...
class EnterpriseBacktestEngine:
    ...
    def calculate_position_size(
        self, symbol: str, price: float, portfolio_value: float, vedic_harmony: float
    ) -> float:
        """
    ...
    def execute_buy(
        ...
        quantity = self.calculate_position_size(
            symbol, price, portfolio_value, vedic_harmony
        )
    ...
    def execute_sell(
        self, symbol: str, price: float, timestamp: str, portfolio_value: float
    ) -> Optional[Trade]:
    ...
    def calculate_risk_metrics(self) -> Dict:
        ...
        annual_return = (
            (self.get_portfolio_value({}) / self.initial_capital) ** (1 / years) - 1
        ) * 100
        max_dd = (
    ...
    def run_backtest(
            ...
            for symbol, price in current_prices.items():
                ...
                if position.quantity == 0:
                    ...
                    if unrealized_pct > 0.10 or unrealized_pct < -0.05:
                        self.execute_sell(
                            symbol, price, str(timestamp), portfolio_value
                        )

            if i % 100 == 0:
                logger.info(
                    f"Progress: {i}/{len(all_dates)} days | Value: ${portfolio_value:,.2f} | DD: {self.current_drawdown:.1%}"
                )
        ...
        for symbol, position in self.positions.items():
            if position.quantity > 0 and symbol in final_prices:
                ...
                self.execute_sell(
                    symbol, final_prices[symbol], str(self.end_date), portfolio_value
                )
        ...
        result = EnterpriseBacktestResult(
            ...
            * 100
            ...
            avg_harmony_at_trade=statistics.mean(self.harmony_scores)
