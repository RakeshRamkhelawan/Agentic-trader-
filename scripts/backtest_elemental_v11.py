...
from backend.agents.elemental_agent_manager_v11 import (
    ElementalAgentManagerV11, MacroSignal, IPO_DATES
)
...
class V11BacktestEngine:
    ...
    def run_backtest(self) -> V11BacktestResult:
        ...
        logger.info("=" * 60)
        logger.info(f"Expected cycles: {len(trading_dates)} days × {len(self.symbols)} symbols = {len(trading_dates) * len(self.symbols):,}")
    ...
    def _partial_close_position(self, symbol: str, exit_price: float, exit_date: datetime,
                                exit_reason: str, price_data: Dict):
