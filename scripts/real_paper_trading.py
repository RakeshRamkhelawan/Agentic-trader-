    ...
    def decide_trade(self, symbol: str, price: float, price_history: List[float]) -> Optional[Dict]:
        ...
        if self.strategy == 'momentum':
            ...
            if deviation < -0.02:  # 2% below average
                return {'side': OrderSide.BUY, 'confidence': 0.7, 'reason': f'below_avg_{deviation:.2%}'}
            elif deviation > 0.02:  # 2% above average
                return {'side': OrderSide.SELL, 'confidence': 0.7, 'reason': f'above_avg_{deviation:.2%}'}
            ...
            if abs(recent_change) > 0.005:  # 0.5% move
                side = OrderSide.BUY if recent_change < 0 else OrderSide.SELL  # Counter-trend scalping
...
class RealPaperTrading:
    ...
    def __init__(self, initial_capital: float = 10000.0):
        ...
        print(f"Target: ALL Bitvavo EUR pairs (400+)")
