...
print(
    f"Actual: {trades} trades / {consensus} consensus = {trades/consensus*100:.1f}% execution of consensus"
)
print(
    f"Actual: {consensus} consensus / {cycles} cycles = {consensus/cycles*100:.2f}% consensus rate"
)
print()
...
for agent in ["fire", "water", "air", "earth", "ether"]:
    ...
    print(
        f"{agent.upper():8} avg={stats['avg_confidence']:.3f} samples={stats['samples']}"
    )
print()
