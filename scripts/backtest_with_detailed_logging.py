...
class TradeExecution:
    ...
    def to_log_entry(self) -> str:
        ...
        for i, decision in enumerate(self.agent_decisions, 1):
            lines.append(
                f"  {i}. {decision.agent_name}: {decision.decision} (confidence: {decision.confidence:.2%})"
            )
...
"""
    ...
    def log_event(
        self, level: LogLevel, category: str, message: str, details: Dict = None
    ):
        """Log a general event"""
    ...
    def log_agent_decision(self, decision: AgentDecision):
        ...
        print(
            f"{action_tag} {decision.agent_name}: {decision.symbol} @ ${decision.suggested_entry_price:,.2f} "
    ...
    def log_trade(self, trade: TradeExecution):
        ...
        print(
            f"{emoji} EXECUTED: {trade.action} {trade.quantity:.4f} {trade.symbol} @ ${trade.price:,.2f}{pnl_str}"
        )
...
"""
    ...
    def log_market_regime_change(
        self, timestamp: str, old_regime: str, new_regime: str, factors: List[str]
    ):
...
def create_sample_agent_decision(symbol: str, timestamp: str) -> AgentDecision:
    ...
    return AgentDecision(
        ...
        primary_motivation="Technical breakout with volume confirmation"
        if decision == "BUY"
        else "Overbought conditions"
        if decision == "SELL"
        else "Awaiting clearer signal",
        secondary_motivations=["Funding rates positive", "Social sentiment improving"],
        risk_assessment="Low volatility regime, position size appropriate"
        if decision != "HOLD"
        else "High uncertainty, reducing exposure",
...
def create_sample_trade(
    symbol: str, timestamp: str, decisions: List[AgentDecision]
) -> TradeExecution:
