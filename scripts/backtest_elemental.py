...
from backend.agents.elemental_agent_manager_v7 import (
    ElementalAgentManagerV7, NavagrahaState, EtherSynthesis
)
...
class ElementalBacktestEngine:
    ...
    def execute_buy(self, symbol: str, price: float, timestamp: str,
                   portfolio_value: float, ether_decision: EtherSynthesis) -> Optional[Trade]:
        ...
        trade = Trade(
            ...
            dominant_planet=ether_decision.cosmic_narrative.split('|')[0] if ether_decision.cosmic_narrative and '|' in ether_decision.cosmic_narrative else ether_decision.cosmic_narrative.split()[0] if ether_decision.cosmic_narrative else "UNKNOWN",
    ...
    def execute_sell(self, symbol: str, price: float, timestamp: str,
        ...
        trade = Trade(
            ...
            dominant_planet=ether_decision.cosmic_narrative.split('|')[0] if ether_decision and ether_decision.cosmic_narrative and '|' in ether_decision.cosmic_narrative else ether_decision.cosmic_narrative.split()[0] if ether_decision and ether_decision.cosmic_narrative else "UNKNOWN",
            fire_decision="APPROVE",
        ...
        self.agent_manager.record_trade_outcome(symbol, realized_pnl or 0, price_change_pct)
    ...
    def run_backtest(self, price_data: Dict[str, List[Dict]]) -> ElementalBacktestResult:
        ...
        logger.info("Agents: Fire(Risk) | Water(Regime) | Air(Signals) | Earth(Value) | Ether(Synthesis)")
        ...
        for i, timestamp in enumerate(all_dates):
            ...
            for symbol, price in current_prices.items():
                ...
                self.harmony_curve.append({
                    ...
                    'planet': ether_decision.cosmic_narrative.split('|')[0] if ether_decision.cosmic_narrative and '|' in ether_decision.cosmic_narrative else ether_decision.cosmic_narrative.split()[0] if ether_decision.cosmic_narrative else "UNKNOWN"
                })
                ...
                if position.quantity == 0:
                    ...
                    if (ether_decision.final_decision == "EXECUTE" and 
                        ether_decision.approved_action == "BUY" and
                        self.cash > price * 0.001):
                        
                        self.execute_buy(symbol, price, timestamp, portfolio_value, ether_decision)
                    ...
                    if (ether_decision.final_decision == "EXECUTE" and 
                        ether_decision.approved_action == "SELL"):
                        self.execute_sell(symbol, price, timestamp, portfolio_value, ether_decision)
            ...
            if i % 100 == 0:
                ...
                avg_harmony = sum(self.harmony_scores[-100:]) / len(self.harmony_scores[-100:]) if self.harmony_scores else 0
                logger.info(f"Progress: {i}/{len(all_dates)} days | Value: ${portfolio_value:,.2f} | "
        ...
        for symbol, position in self.positions.items():
            if position.quantity > 0 and symbol in final_prices:
                ...
                self.execute_sell(symbol, final_prices[symbol], str(self.end_date), portfolio_value, dummy_decision)
        ...
        result = ElementalBacktestResult(
            ...
            avg_harmony_score=statistics.mean(self.harmony_scores) if self.harmony_scores else 0,
    ...
    def _print_summary(self, result: ElementalBacktestResult):
        ...
        print(f"  Harmony Range: {result.min_harmony_score:.3f} - {result.max_harmony_score:.3f}")
        ...
        for agent, stats in result.agent_stats.items():
            if isinstance(stats, dict) and 'avg_confidence' in stats:
                print(f"  {agent.capitalize():8} Avg: {stats['avg_confidence']:.3f} | "
        ...
        if 'consensus_achieved_pct' in result.agent_stats:
            print(f"\n[V3 CONSENSUS STATS]")
            print(f"  Consensus Achieved: {result.agent_stats['consensus_achieved_pct']:.1f}%")
...
if __name__ == "__main__":
    ...
    from datetime import datetime, timedelta
    price_data = {
        "BTC": [{"timestamp": datetime(2020, 1, 1) + timedelta(days=i), "close": 7000 + i * 10 + (i % 10) * 50} for i in range(100)],
        "ETH": [{"timestamp": datetime(2020, 1, 1) + timedelta(days=i), "close": 130 + i * 0.5 + (i % 5) * 5} for i in range(100)],
    }
