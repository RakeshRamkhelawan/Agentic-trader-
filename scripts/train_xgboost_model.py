...
def create_labels(
    df: pd.DataFrame, lookahead: int = 1, threshold: float = 0.005
) -> pd.DataFrame:
    """
...
async def prepare_training_data(
    data_path: str, symbol: str, birth_date: datetime, use_http_fallback: bool = False
...
def train_model(
    X: np.ndarray, y: np.ndarray, output_path: str, validation_split: float = 0.2
) -> Dict:
    """
...
def main():
    ...
    parser.add_argument("--data", "-d", required=True, help="Path to OHLCV CSV file")
    parser.add_argument(
        "--symbol", "-s", required=True, help="Asset symbol (BTC, ETH, etc.)"
    )
    parser.add_argument(
        "--output",
        "-o",
    ...
    X, y = asyncio.run(
        prepare_training_data(args.data, args.symbol, birth_date, args.use_http)
    )
