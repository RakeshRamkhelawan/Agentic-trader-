...
logger = logging.getLogger("BacktestV12")

from backend.agents.elemental_agent_manager_v12 import (
    ElementalAgentManagerV12, MacroSignal, IPO_DATES, HEDGE_PAIRS, INVERSE_ETFS
)
...
class V12BacktestEngine:
    ...
    def __init__(
        ...
        self.agent_manager = ElementalAgentManagerV12()
        ...
        logger.info("=" * 60)
        logger.info("V12: HEDGE PAIRS + BOND INVERSE + SENTIMENT")
        logger.info("=" * 60)
    ...
    def run_backtest(self) -> V12BacktestResult:
        ...
        logger.info("\n" + "=" * 60)
        logger.info("STARTING V12 BACKTEST")
        logger.info("=" * 60)
    ...
    def _process_day(self, trading_date: datetime, price_data: Dict[str, List[Dict]]):
        """V9: Process one day with Position Review Loop"""
        ...
        # V10: Time-based exits first
        self._check_time_based_exits(trading_date, price_data)
        
        # V10: Agent-based position review
        for symbol in list(self.open_positions.keys()):
            ...
            # V10: Position Review (with current_date)
        ...
        # V12: Track which primary symbols got entries for hedge pairing
        primary_entries = []
        
        for symbol in self.symbols:
            # V12: Skip inverse ETFs - they are handled separately after primary entries
        ...
        # ═══════════════════════════════════════════════════════════
        # STAP 3: EVALUATE HEDGE ENTRIES (V12)
        # ═══════════════════════════════════════════════════════════
    ...
    def _generate_result(self, price_data: Dict) -> V12BacktestResult:
        ...
        logger.info("\n" + "=" * 60)
        logger.info("V12 BACKTEST RESULTS")
        logger.info("=" * 60)
        ...
        return V12BacktestResult(
            session_id=f"v12_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
...
def main():
    ...
    output_file = f"backtest_v12_2021_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
