...
from backend.agents.elemental_agent_manager_v16 import (
    INVERSE_ETFS, ElementalAgentManagerV16)

logging.basicConfig(
    level=logging.INFO, format="%(asctime)s | %(levelname)s | %(message)s"
)
logger = logging.getLogger("BacktestV16")
...
class V16BacktestEngine:
    ...
    def __init__(
        ...
        self.agent_manager = ElementalAgentManagerV16()
    ...
    def run_backtest(self):
        ...
        logger.info("=" * 70)
        logger.info("V16: THRESHOLD CALIBRATION")
        logger.info("=" * 70)
        logger.info("Priorities:")
        logger.info("  1. Fire floor: 0.35 (was 0.50)")
        logger.info("  2. Earth floor: 0.45 (was 0.55)")
        logger.info("  3. Ether harmony: 0.45 (was 0.50)")
        logger.info("  4. Planet thresholds: Lowered marginally")
        logger.info("  5. AAVE removed (data anomaly)")
        logger.info("Retained from V15:")
        logger.info("  - Trailing stop, Position cap €2k, 60-day failsafe")
        logger.info("=" * 70)
        ...
        logger.info("\n" + "=" * 60)
        logger.info("STARTING V16 BACKTEST")
        logger.info("=" * 60)
        ...
        logger.info("\n" + "=" * 60)
        logger.info("V16 BACKTEST RESULTS")
        logger.info("=" * 60)
        ...
        for reason, count in sorted(
            self.exit_reasons.items(), key=lambda x: x[1], reverse=True
        ):
        ...
        for symbol, sizes in self.symbol_position_sizes.items():
            if sizes:
                ...
                max_size = max(sizes)
                logger.info(
                    f"  {symbol:8} avg=${avg_size:8.2f}, max=${max_size:8.2f}, n={len(sizes)}"
                )

        return V16BacktestResult(
...
if __name__ == "__main__":
    ...
    if result:
        result.save(
            f"backtest_v16_smoke_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
