...
class TradingAgent:
    ...
    def decide_trade(
        self, symbol: str, price: float, price_history: List[float]
    ) -> Optional[Dict]:
...
class LivePaperTradingProduction:
    ...
    async def initialize(self):
        ...
        try:
            ...
            if success:
                ...
                majors = [
                    p
                    for p in eur_pairs
                    if any(x in p for x in ["BTC", "ETH", "SOL", "XRP", "ADA"])
                ]
        ...
        try:
            ...
            if connected:
                ...
                major_pairs = [
                    s for s in symbols if any(x in s for x in ["BTC", "ETH", "SOL"])
                ]
        ...
        await paper_trading_broadcaster.broadcast_session_start(
            {
            ...
            for symbol in self.symbols["bitvavo"]:
                try:
                    ...
                    if ticker and ticker.get("last"):
                        ...
                        # Broadcast price
                        await paper_trading_broadcaster.broadcast_price_update(
                            symbol, price, "bitvavo"
                        )
                except:
                    pass
    ...
    async def trading_cycle(self):
        ...
        for exchange, symbol in to_trade:
            ...
            if decision and decision["confidence"] >= agent.min_confidence:
                ...
                await paper_trading_broadcaster.broadcast_agent_decision(
                    agent.name,
                    {
                        "symbol": symbol,
    ...
    async def _execute_trade(
        ...
        if result.status.value == "FILLED":
            ...
            # Broadcast trade
            await paper_trading_broadcaster.broadcast_trade(trade)
    ...
    async def broadcast_portfolio(self):
        ...
        await paper_trading_broadcaster.broadcast_portfolio_update(
            {
    ...
    async def broadcast_vedic_context(self):
        ...
        prana_levels = {
            name: agent.prana for name, agent in self.elemental_agents.items()
        }
        ...
        avg_prana = (
            sum(prana_levels.values()) / len(prana_levels) if prana_levels else 50
        )
        ...
        await paper_trading_broadcaster.broadcast_harmony_update(
            self.current_harmony, synthesis
        )

        logger.debug(
            f"[VEDIC] Harmony: {self.current_harmony:.2f}, Prana: {prana_levels}"
        )
    ...
    async def stats_reporter(self, interval: int = 30):
        ...
        while self.running:
            ...
            elapsed = (
                datetime.now(UTC) - self.start_time if self.start_time else timedelta(0)
            )
            total_value, pnl, pnl_pct = await self.broadcast_portfolio()

            # Broadcast stats
            await paper_trading_broadcaster.broadcast_stats(
                {
                    "elapsed": str(elapsed),
        ...
        await paper_trading_broadcaster.broadcast_session_end(
            {
                "total_trades": self.stats["total_trades"],
                "final_pnl": sum(
                    t.get("value", 0) * (1 if t["side"] == "SELL" else -1)
    ...
    def save(self):
        ...
        filename = (
            f"live_paper_session_{datetime.now(UTC).strftime('%Y%m%d_%H%M%S')}.json"
        )
