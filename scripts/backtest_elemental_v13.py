...
logger = logging.getLogger("BacktestV13")

from backend.agents.elemental_agent_manager_v13 import (
    ElementalAgentManagerV13, MacroSignal, IPO_DATES, HEDGE_PAIRS, INVERSE_ETFS
)
...
class V13BacktestEngine:
    ...
    def __init__(
        ...
        self.open_positions: Dict[str, Dict] = {}  # symbol -> {entry_price, entry_date, quantity}
        
        self.trades: List[Trade] = []
        self.equity_curve: List[Dict] = []
        self.harmony_curve: List[Dict] = []
        
        self.agent_manager = ElementalAgentManagerV13()
        ...
        self.hedge_entries = 0
        self.hedge_pairs_active: Dict[str, str] = {}  # primary -> hedge
        ...
        logger.info("=" * 60)
        logger.info("V13: CYCLE FIX + COIN THRESHOLD FIX")
        logger.info("=" * 60)
    ...
    def run_backtest(self) -> V13BacktestResult:
        ...
        logger.info("\n" + "=" * 60)
        logger.info("STARTING V13 BACKTEST")
        logger.info("=" * 60)
        ...
        for trading_date in trading_dates:
            ...
            if cycle_count % 200 == 0:
                portfolio_val = self._calculate_portfolio_value(price_data, trading_date)
                open_count = len(self.open_positions)
                logger.info(f"Day {cycle_count}/{len(trading_dates)} | Portfolio: ${portfolio_val:,.2f} | Open: {open_count}")
        
        return self._generate_result(price_data)
    
    def _process_day(self, trading_date: datetime, price_data: Dict[str, List[Dict]]):
        """V13: Process one day with Position Review Loop + fixed cycle counting"""
        ...
        # V13: Time-based exits first
        self._check_time_based_exits(trading_date, price_data)
        
        # V13: Agent-based position review
        for symbol in list(self.open_positions.keys()):
            ...
            # V13: Position Review (with current_date)
            should_exit, exit_reason = self.agent_manager.evaluate_open_position(
        ...
        # V13: Track which primary symbols got entries for hedge pairing
        primary_entries = []
        
        # V13: Evaluate entry opportunities
        for symbol in self.symbols:
            # Skip inverse ETFs - they are handled separately after primary entries
            ...
            # V13: Process entry cycle (cycle counting done inside agent)
        ...
        # ═══════════════════════════════════════════════════════════
        # STAP 3: EVALUATE HEDGE ENTRIES (V13)
        # ═══════════════════════════════════════════════════════════
        for primary_symbol in primary_entries:
            # Check if hedge signal exists
            ...
            if hedge_sym and hedge_sym not in self.open_positions:
                ...
                # Calculate hedge harmony (lower threshold for hedges)
                planets = ["SUN", "MOON", "MARS", "MERCURY", "JUPITER", "VENUS", "SATURN"]
                dominant_planet = planets[trading_date.day % 7]
                
                # Synthesize with only water confidence for hedge
                fire_conf = self.agent_manager.fire_agent.calculate_confidence(hedge_sym)
                water_conf = hedge_conf
                air_conf = self.agent_manager.air_agent.calculate_confidence(hedge_sym, trading_date)
                earth_conf = self.agent_manager.earth_agent.calculate_confidence(hedge_sym)
                
                harmony, consensus = self.agent_manager.ether_orchestrator.synthesize(
                    fire_conf, water_conf, air_conf, earth_conf
                )
                
                # Lower threshold for hedges (0.55 instead of planet threshold)
                if consensus and harmony >= 0.55:
                    portfolio_value = self._calculate_portfolio_value(price_data, trading_date)
                    
                    # Hedge size = 50% of normal position
                    position_size = self.agent_manager.fire_agent.calculate_position_size(
                        hedge_sym, portfolio_value, harmony, dominant_planet
                    ) * 0.5
                    
                    entry_price = current_price_hedge * (1 + self.agent_manager.SLIPPAGE_PCT)
                    commission = position_size * self.agent_manager.COMMISSION_PCT
                    actual_size = position_size - commission
                    quantity = actual_size / entry_price
                    
                    if quantity > 0 and self.cash >= entry_price * quantity:
                        self._execute_entry({
                            "symbol": hedge_sym,
                            "action": "BUY",
                        ...
                        }, trading_date, price_data)
                        
                        self.hedge_entries += 1
                        self.hedge_pairs_active[primary_symbol] = hedge_sym
    ...
    def _get_macro_signal(self, price_data: Dict, trading_date: datetime) -> MacroSignal:
    ...
    def _close_position(self, symbol: str, exit_price: float, exit_date: datetime,
                       exit_reason: str, price_data: Dict):
        ...
        holding_days = (exit_date - entry_date).days if isinstance(entry_date, datetime) else 0
    ...
    def _calculate_portfolio_value(self, price_data: Dict, target_date: datetime) -> float:
    ...
    def _generate_result(self, price_data: Dict) -> V13BacktestResult:
        ...
        avg_trade_pnl = sum(t.realized_pnl or 0 for t in sell_trades) / total_trades if total_trades > 0 else 0
        ...
        logger.info("\n" + "=" * 60)
        logger.info("V13 BACKTEST RESULTS")
        logger.info("=" * 60)
        ...
        logger.info(f"Hedge Entries:        {self.hedge_entries}")
        ...
        return V13BacktestResult(
            session_id=f"v13_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
...
def main():
    ...
    output_file = f"backtest_v13_2021_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
