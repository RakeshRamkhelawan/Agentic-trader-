...
from backend.agents.elemental_agent_manager_v13 import (
    INVERSE_ETFS, ElementalAgentManagerV13,
    MacroSignal)
    ...
    def _process_day(self, trading_date: datetime, price_data: Dict[str, List[Dict]]):
        ...
        for primary_symbol in primary_entries:
            ...
            if hedge_sym and hedge_sym not in self.open_positions:
                ...
                fire_conf = self.agent_manager.fire_agent.calculate_confidence(
                    hedge_sym
                )
                water_conf = hedge_conf
                air_conf = self.agent_manager.air_agent.calculate_confidence(
                    hedge_sym, trading_date
                )
                earth_conf = self.agent_manager.earth_agent.calculate_confidence(
                    hedge_sym
                )
                ...
                if consensus and harmony >= 0.55:
                    ...
                    position_size = (
                        self.agent_manager.fire_agent.calculate_position_size(
                            hedge_sym, portfolio_value, harmony, dominant_planet
                        )
                        * 0.5
                    )

                    entry_price = current_price_hedge * (
                        1 + self.agent_manager.SLIPPAGE_PCT
                    )
