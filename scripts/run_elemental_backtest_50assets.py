...
def main():
    ...
    print(f"  V7: Calibrated adaptive - warm-start confidence per asset class")
    
    # Group by asset type
    crypto = [s for s in all_symbols if s in ['BTC', 'ETH', 'SOL', 'ADA', 'DOT', 'LINK', 'AVAX', 'MATIC']]
    equities = [s for s in all_symbols if s not in crypto and not any(x in s for x in ['EEM', 'EFA', 'GLD', 'IWM', 'QQQ', 'SPY', 'TLT', 'USO', 'VTI', 'VIX'])]
    etfs = [s for s in all_symbols if s in ['EEM', 'EFA', 'GLD', 'IWM', 'QQQ', 'SPY', 'TLT', 'USO', 'VTI']]
    
    print(f"  Crypto:   {crypto}")
    print(f"  Equities: {equities[:15]}..." if len(equities) > 15 else f"  Equities: {equities}")
    print(f"  ETFs:     {etfs}")
    ...
    # Create summary CSVs
    create_trade_csv(result, f"elemental_backtest_50assets_{result.session_id}_trades.csv")
    create_harmony_csv(result, f"elemental_backtest_50assets_{result.session_id}_harmony.csv")
    create_symbol_performance_csv(result, f"elemental_backtest_50assets_{result.session_id}_symbols.csv")
...
def create_trade_csv(result, filename):
    ...
    with open(filename, 'w', newline='') as f:
        ...
        writer.writerow([
            ...
            'value', 'realized_pnl', 'realized_pnl_pct', 'is_winner', 'is_loser',
            'harmony_score', 'dominant_planet', 'fire_decision', 'ether_decision',
...
def create_harmony_csv(result, filename):
    ...
    with open(filename, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['timestamp', 'symbol', 'harmony', 'decision', 'action', 'planet'])
