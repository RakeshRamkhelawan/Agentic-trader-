...
class PaperBacktestRunner:
    ...
    async def _process_symbol(self, symbol: str):
        ...
        async for tick in self.engine.stream_ticks(symbol):
            ...
            # Log progress
            if tick_count % 500 == 0:
                logger.info(f"  {symbol}: {tick_count} ticks processed, "
                          f"{len([t for t in self.trades if t['symbol'] == symbol])} trades")
        
        logger.info(f"{symbol}: {tick_count} ticks, "
                   f"{len([t for t in self.trades if t['symbol'] == symbol])} trades")
    ...
    def _record_equity(self, timestamp: datetime):
        ...
        self.equity_curve.append({
            ...
            "positions_value": portfolio_value - self.paper_trading.get_cash_balance()
        })
    ...
    def _calculate_results(self) -> BacktestResult:
        ...
        vedic_metrics = {
            ...
            "avg_harmony_score": sum(s.get("harmony", 0) for s in self.vedic_snapshots) / max(len(self.vedic_snapshots), 1),
            "rahu_kala_blocks": sum(1 for s in self.vedic_snapshots if s.get("rahu_kala_active", False))
        }
    ...
    def _print_summary(self, result: BacktestResult):
        ...
        print(f"Symbols:         {', '.join(result.symbols[:5])}{'...' if len(result.symbols) > 5 else ''}")
        print(f"\nCapital:")
