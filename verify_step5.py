...
for name, agent in agents.items():
    ...
    tamas = guna["tamas"]
    print(
        f"   [{status}] {name:8}: sattva={sattva:.1f}, rajas={rajas:.1f}, tamas={tamas:.1f} | dominant: {dominant}"
    )
...
async def test_harmony():
    ...
    result = await orchestrator.process_signal(
        {"inputs": inputs, "soul_context": {"market_regime": "expansion"}}
    )
...
async def test_prana_decay():
    ...
    print(
        "   [OK] Prana consumption working"
        if success
        else "   [FAIL] Prana consumption failed"
    )
