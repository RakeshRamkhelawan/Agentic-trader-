...
async def diagnose_rls():
    ...
    try:
        ...
        if user_info['rolsuper'] or user_info['rolbypassrls']:
             ...
             print("    Solution: Create a separate non-superuser account for the application.")
        else:
        ...
        await conn.execute("SELECT set_config('app.current_tenant', 'debug-tenant', false)")
        setting = await conn.fetchval("SELECT current_setting('app.current_tenant', true)")
