...
class SignalGenerator:
    ...
    def generate_statistical_signal(
        ...
        stat_significant = (
            stationarity_result.get("significant", False)
        ...
        indicators = [
            ...
            SignalIndicator(
                name="Stationarity",
                value=(
                    stationarity_result.get("statistic", 0)
                    if stationarity_result
                    else 0
                ),
                threshold=0.0,
    ...
    def generate_signals(
        ...
        if "spread_metrics" in analysis_results:
            spread_signal = self.generate_spread_signal(
                market, symbol, analysis_results["spread_metrics"], category
            )
            if spread_signal:
        ...
        if "volume_metrics" in analysis_results:
            volume_signal = self.generate_volume_signal(
                market, symbol, analysis_results["volume_metrics"], category
            )
            if volume_signal:
        ...
        if "test_results" in analysis_results:
            stat_signal = self.generate_statistical_signal(
                market, symbol, analysis_results["test_results"], category
            )
            if stat_signal:
    ...
    def rank_signals(self, signals: List[MarketSignal]) -> List[MarketSignal]:
        ...
        """
        return sorted(signals, key=lambda s: (s.confidence, s.score), reverse=True)
