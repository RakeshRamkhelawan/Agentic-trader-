...
class StatisticalTestsFramework:
    ...
    def test_normality(
        ...
        if test_method == "shapiro":
            ...
            statistic, p_value = scipy_stats.kstest(data, "norm", args=(data.mean(), data.std()))
    ...
    def test_stationarity(self, data: pd.Series) -> TestResult:
        ...
        significant = p_value < self.alpha
        interpretation = "Stationary (reject H0)" if significant else "Non-stationary (unit root)"
