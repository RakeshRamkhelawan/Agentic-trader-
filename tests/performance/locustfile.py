...
class PredictionServiceUser(HttpUser):
    ...
    def get_signals_paginated(self):
        ...
        with self.client.get(
            "/api/v1/signals?limit=5&offset=10",
            catch_response=True
        ) as response:
            if response.status_code == 200:
                response.success()
...
def on_test_stop(environment, **kwargs):
    ...
    if total_requests > 0:
        ...
        logger.info("\nPERFORMANCE TARGETS:")
        logger.info("  P50 latency < 50ms: " + ("✓ PASS" if stats.total.avg_response_time < 50 else "✗ FAIL"))
        logger.info("  P95 latency < 200ms: " + ("✓ PASS" if stats.total.get_response_time_percentile(0.95) < 200 else "✗ FAIL"))
        logger.info("  P99 latency < 500ms: " + ("✓ PASS" if stats.total.get_response_time_percentile(0.99) < 500 else "✗ FAIL"))
        logger.info("  Error rate < 1%: " + ("✓ PASS" if failure_rate < 1 else "✗ FAIL"))
        logger.info("=" * 80)
