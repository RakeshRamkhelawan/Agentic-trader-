    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"{API_URL}/api/v1/paper-trading/status", timeout=5
            ) as resp:
                if resp.status == 200:
                    data = await resp.json()
...
async def test_vedic_status():
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"{API_URL}/api/v1/paper-trading/vedic-status", timeout=5
            ) as resp:
                if resp.status == 200:
                    data = await resp.json()
                    ...
                    print(
                        f"   [INFO] Status: {resp.status} - Endpoint mogelijk niet geimplementeerd"
                    )
                    return False
    except Exception as e:
