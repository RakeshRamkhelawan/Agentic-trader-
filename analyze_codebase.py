...
def analyze_api_endpoints(api_dir):
    ...
    for file in api_files:
        ...
        routes = re.findall(r'@router\.(get|post|put|delete|patch|websocket)\(["\']([^"\']+)', content, re.IGNORECASE)
        if routes:
            endpoints[file.name] = routes
            
        ws_patterns = re.findall(r'websocket|SSE|EventSource|async def.*stream', content, re.IGNORECASE)
        if ws_patterns:
...
def parse_phase_document(doc_path):
    ...
    phase_sections = re.findall(r'##\s+(Phase\s+\d+[:\s]+[^\n]+)(.*?)(?=##\s+Phase|\Z)', content, re.DOTALL)
...
def main():
    ...
    if master_kanban.exists():
        ...
        for phase_id, data in list(phases.items())[:3]:
            ...
            print(f"  Tasks: {len(data['tasks'])}, Microtasks: {data['microtask_count']}")
    
    if fase_01.exists():
        content = read_file_safe(fase_01)
        microtasks = re.findall(r'####\s+Microtask\s+([\d\.]+)', content)
        print(f"\n\nFASE_01: {len(microtasks)} microtasks detected")
        print(f"  Range: {microtasks[0] if microtasks else 'N/A'} to {microtasks[-1] if microtasks else 'N/A'}")
